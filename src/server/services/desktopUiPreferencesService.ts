...
import { appStateDir } from '../../constants/appPaths.js'
...
const DEFAULT_PROFILE_SUBTITLE = 'github.com/SocialAI-X/Auto-ABM'
...
export class DesktopUiPreferencesService {
  ...
  private getPreferencesPath(): string {
    return path.join(appStateDir(), 'desktop-ui.json')
  }

  private getProfileDir(): string {
    return path.join(appStateDir(), 'profile')
  }
  ...
  private getProfileAvatarPath(avatarFile: string): string {
    ...
    return path.join(appStateDir(), normalized)
  }
  ...
  async readPreferences(): Promise<DesktopUiPreferencesReadResult> {
    ...
    const preferences = await readRecoverableJsonFile({
      filePath,
      label: 'AutoABM desktop UI preferences',
