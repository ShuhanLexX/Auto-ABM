...
async function migrateLegacyStateNamespace(
  configDir: string,
  report: MigrationReport,
): Promise<string> {
  const nextDir = appStateDir(configDir)
  const legacyDir = path.join(configDir, LEGACY_STATE_DIR_NAME)
  ...
  try {
    ...
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`${LEGACY_STATE_DIR_NAME} -> ${path.basename(nextDir)}: ${error instanceof Error ? error.message : String(error)}`)
      return nextDir
    }
  }
  ...
  try {
    ...
    await fs.rename(legacyDir, nextDir)
    report.migratedEntries.push(`${LEGACY_STATE_DIR_NAME} -> ${path.basename(nextDir)}`)
    return nextDir
  } catch (error) {
    if (errnoCode(error) === 'ENOENT') return nextDir
    try {
      await fs.cp(legacyDir, nextDir, { recursive: true })
      report.migratedEntries.push(`${LEGACY_STATE_DIR_NAME} -> ${path.basename(nextDir)} (copy)`)
    } catch (copyError) {
      report.failures.push(
        `${LEGACY_STATE_DIR_NAME} -> ${path.basename(nextDir)}: ${copyError instanceof Error ? copyError.message : String(copyError)}`,
      )
    }
    return nextDir
  }
...
async function migrateLegacyRootProviders(
  ...
  const targetPath = path.join(appStateRoot, 'providers.json')
  try {
    await fs.access(targetPath)
    return
  } catch (error) {
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`${LEGACY_STATE_DIR_NAME}/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    report.migratedEntries.push(`providers.json -> ${LEGACY_STATE_DIR_NAME}/providers.json`)

    const settingsPath = path.join(appStateRoot, 'settings.json')
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push(`providers.json -> ${LEGACY_STATE_DIR_NAME}/settings.json`)
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
...
// ABM persistence lives under autoabm/abm/. Keep in sync with
// src/server/abm/types.ts::ABM_STORAGE_VERSION and storagePaths.ts layout.
const ABM_STORAGE_SCHEMA_VERSION = 1

async function migrateAbmStorage(
  appStateRoot: string,
  report: MigrationReport,
): Promise<void> {
  const projectsIndexPath = path.join(appStateRoot, 'abm', 'projects.json')
  try {
    const current = await readJsonFile(projectsIndexPath)
    if (!current.missing) return
    await writeJsonFile(projectsIndexPath, {
      schemaVersion: ABM_STORAGE_SCHEMA_VERSION,
      projects: [],
    })
    report.migratedEntries.push('autoabm/abm/projects.json (init)')
  } catch (error) {
    report.failures.push(
      `autoabm/abm/projects.json: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
...
async function runPersistentStorageMigrations(configDir: string): Promise<MigrationReport> {
  ...
  const appStateRoot = await migrateLegacyStateNamespace(configDir, report)

  await migrateLegacyRootProviders(configDir, appStateRoot, report)

  await migrateJsonEntry(
    path.join(appStateRoot, 'providers.json'),
    `${LEGACY_STATE_DIR_NAME}/providers.json`,
    report,
    migrateProvidersIndex,
  )
  await migrateJsonEntry(
    path.join(appStateRoot, 'settings.json'),
    `${LEGACY_STATE_DIR_NAME}/settings.json`,
    report,
    migrateManagedSettings,
  )

  await migrateAbmStorage(appStateRoot, report)
