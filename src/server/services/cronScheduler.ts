...
function getLogFilePath(): string {
  return path.join(getAppConfigHomeDir(), 'scheduled_tasks_log.json')
}
...
export class CronScheduler {
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const appStateRoot = appStateDir(this.getConfigDir())
    if (existsSync(path.join(appStateRoot, 'providers.json'))) {
      return true
    }

    try {
      const raw = readFileSync(path.join(appStateRoot, 'settings.json'), 'utf-8')
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    try {
      const raw = readFileSync(
        path.join(appStateDir(this.getConfigDir()), 'settings.json'),
        'utf-8',
      )
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
