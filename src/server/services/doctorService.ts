...
export class DoctorService {
  ...
  constructor(options: DoctorServiceOptions = {}) {
    this.configDir = options.configDir || getAppConfigHomeDir()
  ...
  private async buildTargets(): Promise<DoctorTarget[]> {
    const targets: DoctorTarget[] = [
      ...
      this.jsonTarget(
        ...
        'Managed providers',
        'user',
        path.join(appStateDir(this.configDir), 'providers.json'),
      ),
      this.jsonTarget(
        'cc-haha-settings',
        'Managed provider settings',
        'user',
        path.join(appStateDir(this.configDir), 'settings.json'),
      ),
      ...
      this.jsonTarget('oauth', 'OAuth tokens', 'user', path.join(appStateDir(this.configDir), 'oauth.json')),
      this.jsonTarget(
        'openai-oauth',
        'OpenAI OAuth tokens',
        'user',
        path.join(appStateDir(this.configDir), 'openai-oauth.json'),
      ),
    ]
