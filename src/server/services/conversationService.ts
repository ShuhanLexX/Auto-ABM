...
export class ConversationService {
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const appStateRoot = appStateDir()
    const providersIndexPath = path.join(appStateRoot, 'providers.json')
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    const settingsPath = path.join(appStateDir(), 'settings.json')
    try {
      const raw = fs.readFileSync(settingsPath, 'utf-8')
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
  ...
  private clearStaleLock(sessionId: string): boolean {
    const lockDir = path.join(getAppConfigHomeDir(), '.lock')
  ...
  private async materializeAttachments(
    ...
    const uploadDir = path.join(getAppConfigHomeDir(), 'uploads', sessionId)
