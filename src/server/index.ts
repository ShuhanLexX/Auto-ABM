/**
 * AutoABM Desktop App — HTTP + WebSocket Server
 *
...
import { abmRunService } from './abm/abmRunService.js'
...
export function startServer(port = PORT, host = HOST) {
  ...
  try {
    server = Bun.serve<WebSocketData>({
      ...
      async fetch(req, server) {
        ...
        // Dedicated ABM frame channel: /ws/abm/:runId (kept separate from the
        // chat socket so simulation frames never contend with chat streaming).
        if (url.pathname.startsWith('/ws/abm/')) {
          if (cors.rejected) {
            return corsRejectedResponse(cors)
          }
          ...
          const abmMatch = url.pathname.match(/^\/ws\/abm\/([0-9a-zA-Z_-]{1,64})$/)
          if (!abmMatch) {
            return new Response('Invalid run ID', { status: 400 })
          }
          const runId = abmMatch[1]!
  ...
  console.log(`[Server] AutoABM API server running at http://${host}:${serverPort}`)
  return server
}
...
export async function stopServerRuntimeForShutdown(
  ...
  // Kill any active ABM kernel subprocesses (analogous to CLI session cleanup).
  await abmRunService.stopAll().catch((error) => {
    console.error('[Server] Failed to stop ABM kernels during shutdown:', error)
  })
