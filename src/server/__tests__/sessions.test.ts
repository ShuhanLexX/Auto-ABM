...
describe('Sessions API', () => {
  ...
  it('POST /api/sessions/project-directory should create a named research directory', async () => {
    const previousRoot = process.env.AUTOABM_RESEARCH_ROOT
    const researchRoot = path.join(tmpDir, 'AutoABM-root')
    process.env.AUTOABM_RESEARCH_ROOT = researchRoot
    try {
      const first = await fetch(`${baseUrl}/api/sessions/project-directory`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'Market / behavior?' }),
      })
      expect(first.status).toBe(201)
      const firstBody = (await first.json()) as { name: string; workDir: string }
      expect(firstBody.name).toBe('Market - behavior')
      expect(firstBody.workDir).toBe(path.join(researchRoot, 'Market - behavior'))
      expect((await fs.stat(firstBody.workDir)).isDirectory()).toBe(true)

      const second = await fetch(`${baseUrl}/api/sessions/project-directory`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'Market / behavior?' }),
      })
      expect(second.status).toBe(201)
      const secondBody = (await second.json()) as { name: string; workDir: string }
      expect(secondBody.name).toBe('Market - behavior-2')
      expect((await fs.stat(secondBody.workDir)).isDirectory()).toBe(true)
    } finally {
      if (previousRoot === undefined) delete process.env.AUTOABM_RESEARCH_ROOT
      else process.env.AUTOABM_RESEARCH_ROOT = previousRoot
    }
  })
