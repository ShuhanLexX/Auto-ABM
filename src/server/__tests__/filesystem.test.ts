...
import { handleFilesystemRoute, setDirectoryDialogRunnerForTests } from '../api/filesystem.js'
...
afterEach(async () => {
  ...
  setDirectoryDialogRunnerForTests(null)
})
...
async function makeExternalFixtureDir(): Promise<string | null> {
  ...
  if (!isWithinPath(workspaceRoot, homeDir)) {
    try {
      return await fsp.mkdtemp(path.join(workspaceRoot, '.filesystem-test-'))
    } catch {
...
describe('filesystem API', () => {
  ...
  it('allows directory picker browsing outside registered filesystem roots without widening normal browse', async () => {
    const externalFixtureDir = await makeExternalFixtureDir()
    if (!externalFixtureDir) return

    cleanupDirs.add(externalFixtureDir)
    await fsp.mkdir(path.join(externalFixtureDir, 'research-question'))
    ...
    const pickerRes = await handleFilesystemRoute(
      '/api/filesystem/browse',
      makeUrl('/api/filesystem/browse', {
        path: externalFixtureDir,
    ...
    const pickerBody = await pickerRes.json() as { entries: Array<{ name: string; isDirectory: boolean }> }
    expect(pickerBody.entries).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'research-question', isDirectory: true }),
    ]))

    const stillDeniedRes = await handleFilesystemRoute(
      '/api/filesystem/browse',
      makeUrl('/api/filesystem/browse', {
        path: externalFixtureDir,
      }),
    )
    expect(stillDeniedRes.status).toBe(403)
  })

  it('returns filesystem roots for directory picker browsing with no selected path', async () => {
    const res = await handleFilesystemRoute(
      '/api/filesystem/browse',
      makeUrl('/api/filesystem/browse', {
        directoryPicker: 'true',
    ...
    expect(res.status).toBe(200)
    const body = await res.json() as { currentPath: string; entries: Array<{ path: string; isDirectory: boolean }> }
    if (process.platform === 'win32') {
      expect(body.currentPath).toBe('')
      expect(body.entries.length).toBeGreaterThan(0)
      expect(body.entries.every((entry) => /^[A-Z]:\\$/i.test(entry.path) && entry.isDirectory)).toBe(true)
    } else {
      expect(body.currentPath).toBe(path.parse(os.homedir() || '/').root || '/')
    }
  })
  ...
  it('returns and registers a folder selected by the native directory dialog', async () => {
    const homeFixtureDir = await fsp.mkdtemp(path.join(os.homedir(), 'claude-filesystem-test-'))
    cleanupDirs.add(homeFixtureDir)
    setDirectoryDialogRunnerForTests(async () => ({ status: 'selected', path: homeFixtureDir }))

    const res = await handleFilesystemRoute(
      '/api/filesystem/open-directory-dialog',
      makeUrl('/api/filesystem/open-directory-dialog', {}),
    )

    expect(res.status).toBe(200)
    const body = await res.json() as { path: string | null }
    expect(body.path).toBe(path.resolve(homeFixtureDir))
    expect(isWithinRegisteredFilesystemRoot(homeFixtureDir)).toBe(true)
  })

  it('passes a bounded timeout to the native directory dialog runner', async () => {
    let seenTimeoutMs = 0
    setDirectoryDialogRunnerForTests(async (_initialPath, timeoutMs) => {
      seenTimeoutMs = timeoutMs ?? 0
    ...
    expect(res.status).toBe(200)
    expect(seenTimeoutMs).toBe(2500)
  })
})
