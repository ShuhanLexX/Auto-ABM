...
import { deleteAllProjectsCascade, deleteProjectsForSession } from '../abm/abmStore.fs.js'
...
export async function handleSessionsApi(
  ...
  try {
    ...
    // Special collection route: /api/sessions/project-directory
    if (sessionId === 'project-directory') {
      if (req.method !== 'POST') {
        return Response.json(
          { error: 'METHOD_NOT_ALLOWED', message: `Method ${req.method} not allowed` },
          { status: 405 }
      ...
      return await createProjectDirectory(req)
    }
...
async function createProjectDirectory(req: Request): Promise<Response> {
  ...
  const directoryName = normalizeProjectDirectoryName(body.name)
  const root = getDefaultResearchRoot()
  await fs.mkdir(root, { recursive: true })
  const workDir = await createUniqueProjectDirectory(root, directoryName)
  registerFilesystemAccessRoot(workDir)
  return Response.json({ name: path.basename(workDir), workDir }, { status: 201 })
}

function getDefaultResearchRoot(): string {
  return process.env.AUTOABM_RESEARCH_ROOT || path.join(os.homedir(), 'AutoABM')
}

function normalizeProjectDirectoryName(value: unknown): string {
  ...
  const sanitized = value
    ...
    .replace(/[-. ]+$/g, '')
    .slice(0, 80)
  const normalized = sanitized || 'simulation'
  ...
  return reservedNames.has(normalized.toUpperCase()) ? `${normalized}-project` : normalized
}

async function createUniqueProjectDirectory(root: string, directoryName: string): Promise<string> {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const candidateName = attempt === 0 ? directoryName : `${directoryName}-${attempt + 1}`
  ...
  throw ApiError.conflict(`Could not create a unique project directory for ${directoryName}`)
}
...
async function deleteSession(sessionId: string): Promise<Response> {
  ...
  await cleanupAbmSessionAssets(sessionId, workDir)
  await cleanupAllAbmAssetsWhenNoSessionsRemain()
...
async function batchDeleteSessions(req: Request): Promise<Response> {
  ...
  await Promise.all(sessionIds.map(async (sessionId) => {
    workDirs.set(sessionId, await sessionService.getSessionWorkDir(sessionId))
  }))
  ...
  for (const sessionId of result.successes) {
    closeSessionConnection(sessionId, 'session deleted')
    cleanupAdapterSessionMappings(sessionId)
    await cleanupAbmSessionAssets(sessionId, workDirs.get(sessionId) ?? null)
  }
  await cleanupAllAbmAssetsWhenNoSessionsRemain()
...
async function cleanupAbmSessionAssets(sessionId: string, workDir: string | null): Promise<void> {
  const deleteUnownedWorkDirProjects = workDir
    ? !(await hasRemainingSessionForWorkDir(workDir))
    : false
  const removed = await deleteProjectsForSession({
    sessionId,
    workDir,
    deleteUnownedWorkDirProjects,
  })
  if (removed.length > 0) {
    console.log(`[Sessions API] Removed ${removed.length} ABM project(s) for ${sessionId}`)
  }
}

async function cleanupAllAbmAssetsWhenNoSessionsRemain(): Promise<void> {
  if (await hasAnyRemainingSession()) return
  const removed = await deleteAllProjectsCascade()
  if (removed.length > 0) {
    console.log(`[Sessions API] Removed ${removed.length} remaining ABM project(s) after all sessions were deleted`)
  }
}

async function hasAnyRemainingSession(): Promise<boolean> {
  const { sessions } = await sessionService.listSessions({ limit: 1 })
  return sessions.length > 0
}

async function hasRemainingSessionForWorkDir(workDir: string): Promise<boolean> {
  const target = path.resolve(workDir)
  const { sessions } = await sessionService.listSessions({ limit: 500 })
  for (const session of sessions) {
    const candidate = session.workDir || session.projectRoot || null
