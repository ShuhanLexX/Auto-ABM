...
const DIRECTORY_DIALOG_TIMEOUT_MS = 10 * 60 * 1000
const DIRECTORY_DIALOG_MIN_TIMEOUT_MS = 1_000
...
function isDirectoryPickerMode(url: URL): boolean {
  return url.searchParams.get('directoryPicker') === 'true'
}

function windowsDriveRoots(): FilesystemEntry[] {
  if (process.platform !== 'win32') return []
  return WINDOWS_DRIVE_LETTERS
    .map((letter) => `${letter}:\\`)
    .filter((drivePath) => {
      try {
        return fs.existsSync(drivePath) && fs.statSync(drivePath).isDirectory()
      } catch {
        return false
      }
    })
    .map((drivePath) => ({
      name: drivePath.replace(/\\$/, ''),
      path: drivePath,
      isDirectory: true,
      relativePath: drivePath.replace(/\\$/, ''),
    }))
}

function directoryPickerRootResponse(): Response {
  ...
  return browseDirectoryForPicker(path.parse(os.homedir() || '/').root || '/')
}

function directoryParentPath(resolvedPath: string): string {
  const root = path.parse(resolvedPath).root
  if (process.platform === 'win32' && resolvedPath.toLowerCase() === root.toLowerCase()) {
    return ''
  }
  return path.dirname(resolvedPath)
}

function browseDirectoryForPicker(targetPath: string): Response {
  const resolvedPath = path.resolve(normalizeDriveRootPathForPlatform(targetPath))
  const parentPath = directoryParentPath(resolvedPath)

  try {
    const stat = fs.statSync(resolvedPath)
    if (!stat.isDirectory()) {
      return json({ error: 'Not a directory', currentPath: resolvedPath, parentPath, entries: [] }, 400)
    }
    ...
    return json({
      ...
      parentPath,
      entries: [],
      error: `Cannot read directory: ${err}`,
    })
  }
}
...
type DirectoryDialogRunner = (initialPath?: string, timeoutMs?: number) => Promise<DirectoryDialogResult>

let directoryDialogRunnerForTests: DirectoryDialogRunner | null = null

export function setDirectoryDialogRunnerForTests(runner: DirectoryDialogRunner | null): void {
  directoryDialogRunnerForTests = runner
}
...
export async function handleFilesystemRoute(pathname: string, url: URL): Promise<Response> {
  ...
  if (pathname === '/api/filesystem/open-directory-dialog') {
    return handleOpenDirectoryDialog(url)
  }
...
async function handleOpenDirectoryDialog(url: URL): Promise<Response> {
  const runner = directoryDialogRunnerForTests ?? openNativeDirectoryDialog
  const initialPath = url.searchParams.get('initialPath') || undefined
  const requestedTimeoutMs = Number(url.searchParams.get('timeoutMs') || '')
  const timeoutMs = Number.isFinite(requestedTimeoutMs) && requestedTimeoutMs > 0
    ? Math.max(DIRECTORY_DIALOG_MIN_TIMEOUT_MS, Math.min(DIRECTORY_DIALOG_TIMEOUT_MS, Math.round(requestedTimeoutMs)))
    : DIRECTORY_DIALOG_TIMEOUT_MS
  const result = await runner(initialPath, timeoutMs)
  ...
  if (result.status === 'unavailable') {
    return json({ error: result.message || 'Native directory picker unavailable' }, 501)
  }

  const resolvedPath = path.resolve(normalizeDriveRootPathForPlatform(result.path))
  try {
    const stat = fs.statSync(resolvedPath)
    if (!stat.isDirectory()) {
      return json({ error: 'Selected path is not a directory' }, 400)
    }
  } catch {
    return json({ error: 'Selected directory does not exist' }, 400)
  }

  registerFilesystemAccessRoot(resolvedPath)
  return json({ path: resolvedPath })
}

async function openNativeDirectoryDialog(initialPath?: string, timeoutMs = DIRECTORY_DIALOG_TIMEOUT_MS): Promise<DirectoryDialogResult> {
  if (process.platform === 'win32') {
    return openWindowsDirectoryDialog(initialPath, timeoutMs)
  }

  if (process.platform === 'darwin') {
    return openMacDirectoryDialog(initialPath, timeoutMs)
  }

  return openLinuxDirectoryDialog(initialPath, timeoutMs)
}

async function openWindowsDirectoryDialog(initialPath?: string, timeoutMs = DIRECTORY_DIALOG_TIMEOUT_MS): Promise<DirectoryDialogResult> {
  const script = [
    '$ErrorActionPreference = "Stop"',
    '$initial = $env:AUTOABM_INITIAL_DIRECTORY',
    'try {',
    '  $shell = New-Object -ComObject Shell.Application',
    '  $folder = $shell.BrowseForFolder(0, "选择研究问题文件夹", 0x51, $initial)',
  ...
  const result = await execFileNoThrowWithCwd('powershell.exe', [
    ...
    env: {
      ...process.env,
      AUTOABM_INITIAL_DIRECTORY: initialPath || '',
    },
  })

  return normalizeDirectoryDialogOutput(result)
}

async function openMacDirectoryDialog(initialPath?: string, timeoutMs = DIRECTORY_DIALOG_TIMEOUT_MS): Promise<DirectoryDialogResult> {
  const prompt = 'Choose project folder'
  const args = initialPath
    ? [
        '-e',
        `set initialFolder to POSIX file "${escapeAppleScriptString(initialPath)}"`,
        '-e',
        `set selectedFolder to choose folder with prompt "${prompt}" default location initialFolder`,
        '-e',
  ...
  const result = await execFileNoThrowWithCwd('osascript', args, {
    timeout: timeoutMs,
  })
...
async function openLinuxDirectoryDialog(initialPath?: string, timeoutMs = DIRECTORY_DIALOG_TIMEOUT_MS): Promise<DirectoryDialogResult> {
  const zenityArgs = ['--file-selection', '--directory', '--title=Choose project folder']
  if (initialPath) zenityArgs.push(`--filename=${initialPath}`)
  const zenity = await execFileNoThrowWithCwd('zenity', zenityArgs, {
    timeout: timeoutMs,
  })
  const zenityResult = normalizeDirectoryDialogOutput(zenity)
  if (zenityResult.status !== 'unavailable') return zenityResult

  const kdialogArgs = ['--getexistingdirectory']
  if (initialPath) kdialogArgs.push(initialPath)
  const kdialog = await execFileNoThrowWithCwd('kdialog', kdialogArgs, {
    timeout: timeoutMs,
  })
  return normalizeDirectoryDialogOutput(kdialog)
}

function normalizeDirectoryDialogOutput(result: { stdout: string; stderr: string; code: number; error?: string }): DirectoryDialogResult {
  const selected = result.stdout.trim()
  if (result.code === 0 && selected) return { status: 'selected', path: selected }
  if (result.code === 0) return { status: 'cancelled' }
  const message = [result.error, result.stderr].filter(Boolean).join('\n')
  if (/not found|ENOENT|command not found|not recognized/i.test(message)) {
    return { status: 'unavailable', message: 'Native directory picker unavailable' }
  }
...
async function handleBrowse(url: URL): Promise<Response> {
  const directoryPicker = isDirectoryPickerMode(url)
  const targetPathParam = url.searchParams.get('path')

  if (directoryPicker && !targetPathParam) {
    return directoryPickerRootResponse()
  }

  const targetPath = targetPathParam || os.homedir() || '/'
  const resolvedPath = path.resolve(normalizeDriveRootPathForPlatform(targetPath))

  if (directoryPicker) {
    return browseDirectoryForPicker(resolvedPath)
  }
