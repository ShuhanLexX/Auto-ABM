...
import { wsAbmHandler } from '../abm/wsAbmHandler.js'
...
export type WebSocketData = {
  sessionId: string
  connectedAt: number
  channel: 'client' | 'sdk' | 'abm'
  sdkToken: string | null
  serverPort: number
  serverHost: string
  // Set only on the dedicated ABM frame channel (/ws/abm/:runId).
  runId?: string
}
...
export const handleWebSocket = {
  open(ws: ServerWebSocket<WebSocketData>) {
    if (ws.data.channel === 'abm') {
      wsAbmHandler.open(ws)
      return
    }
