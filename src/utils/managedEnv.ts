...
function getCcHahaSettingsEnv(): Record<string, string> {
  try {
    const appStateSettingsPath = join(appStateDir(), 'settings.json')
    const raw = readFileSync(appStateSettingsPath, 'utf-8')
    const parsed = JSON.parse(raw) as { env?: Record<string, string> }
    const settingsEnv = normalizeLegacyDeepSeekManagedEnv(parsed.env ?? {}).env
    return mergeActiveProviderManagedEnv(settingsEnv, getAppConfigHomeDir())
  } catch {
    return mergeActiveProviderManagedEnv({}, getAppConfigHomeDir())
  }
}
