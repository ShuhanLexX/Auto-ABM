...
```bash
./bin/autoabm                          # Interactive TUI mode
./bin/autoabm -p "your prompt here"    # Headless mode
./bin/autoabm --help                   # Show all options
```
...
```bash
CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/autoabm
```
