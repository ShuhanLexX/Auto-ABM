# AutoABM Channel System Documentation
...
### [01-channel-system.md](./01-channel-system.md) â€?Channel System Architecture
...
- **Six-Layer Access Control**: Capability â†?Runtime gate â†?OAuth â†?Org policy â†?Session allowlist â†?Plugin approval
...
All illustrations use a dark background (#1a1a2e) with AutoABM orange-blue accents (#FF7A00).

| Image | Description | Document |
|-------|-------------|----------|
| `01-channel-overview.png` | Channel system architecture overview | Architecture |
| `02-message-flow.png` | End-to-end message flow: IM â†?Agent â†?IM | Architecture |
...
4. Key source files:
   - `src/services/mcp/channelNotification.ts` â€?Core gating and message wrapping
   - `src/services/mcp/channelPermissions.ts` â€?Permission relay system
   - `src/services/mcp/channelAllowlist.ts` â€?Allowlist management
   - `src/utils/plugins/mcpPluginIntegration.ts` â€?Plugin MCP integration
   - `src/utils/plugins/schemas.ts` â€?Plugin manifest Channel schema
