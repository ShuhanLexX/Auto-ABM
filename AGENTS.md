...
This file is the **engineering entry** for AI coding agents in this repository. Treat it as executable guidance: inspect the real code, make narrow changes, verify the affected behavior, and leave a handoff that another maintainer can trust.

> **Product**: **Auto-ABM** is an AI-native ABM research workbench (Simulation-first UX, not a code-centric IDE). Product truth lives in `core-requirements.md`; AI-oriented routing lives in `docs/ai/`.

## AI Doc Map (read before large features)

| Order | Document | Purpose |
| --- | --- | --- |
| 1 | [`core-requirements.md`](./core-requirements.md) | Product objects, modes, MVP scope, non-negotiables |
| 2 | [`docs/ai/INDEX.md`](./docs/ai/INDEX.md) | Full AI doc index and conflict resolution |
| 3 | [`docs/ai/FORK-CONTEXT.md`](./docs/ai/FORK-CONTEXT.md) | Platform capabilities vs ABM product domain; reuse vs new modules |
| 4 | [`docs/ai/PRODUCT-CONSTRAINTS.md`](./docs/ai/PRODUCT-CONSTRAINTS.md) | Decision guardrails while coding |
| 5 | [`docs/ai/SURFACE-ROUTING.md`](./docs/ai/SURFACE-ROUTING.md) | Which surface owns ABM concerns |
| 6 | [`docs/ai/TASK-PROTOCOL.md`](./docs/ai/TASK-PROTOCOL.md) | Boss tasks, escalation, handoff format |
| 7 | [`docs/ai/IMPLEMENTATION.md`](./docs/ai/IMPLEMENTATION.md) | **Implementation entry**: subsystem map, cross-cutting contracts, phasing → `docs/ai/impl/*` (kernel bridge, simulation canvas, conversation UX, roadmap) |
| 8 | **This file** | Engineering contract below |
| — | [`.cursor/rules/`](./.cursor/rules/) | Persistent Cursor rule summaries |

**Reuse before building**: `packages/abm-kernel` (Python Mesa engine) is the simulation source of truth — reuse it via subprocess, do not reimplement it in TS. See `docs/ai/impl/kernel-bridge.md`.

**Not ABM product spec**: `docs/agent/`, `docs/memory/`, `docs/skills/`, `docs/desktop/` (human/base-platform docs). Use them only when touching the underlying Coding Agent mechanisms.

**Conflict order**: Boss instruction → `core-requirements.md` → `docs/ai/*` → this file → legacy base docs.
...
- Start every task by identifying the changed surface: `desktop`, `server`, `adapter`, `native`, `docs`, `provider/runtime`, `agent-loop`, `release`, or ABM-specific work per `docs/ai/SURFACE-ROUTING.md` (maps to `desktop` / `server` / `agent-loop`).
...
- `bin/autoabm` is the executable entrypoint; `bun run start` and `./bin/autoabm` run the CLI locally.
...
- `./bin/autoabm` or `bun run start`: run the CLI locally.
...
- Desktop Doctor and any automatic repair path must be deny-by-default. One-click repair may only mutate allowlisted, regenerable desktop UI state such as `autoabm-*` `localStorage` keys or native window state.
...
- Protected files include `~/.claude/projects/**/*.jsonl`, `~/.claude/settings.json`, project `.claude/settings.json`, `~/.claude/autoabm/providers.json`, `~/.claude/autoabm/settings.json`, `~/.claude/adapters.json`, `~/.claude/adapter-sessions.json`, `~/.claude/skills`, project `.claude/skills`, `.mcp.json`, managed MCP config, `~/.claude/plugins/**`, `~/.claude/teams/**`, and `~/.claude/autoabm/*oauth*.json`. Diagnose these paths only with redaction by default.
