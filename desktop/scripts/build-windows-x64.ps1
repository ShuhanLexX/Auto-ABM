...
function Clear-Directory {
  param([string]$Path)
  if (Test-Path $Path) {
    $emptyDir = Join-Path ([System.IO.Path]::GetTempPath()) "autoabm-empty-$([Guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Force -Path $emptyDir | Out-Null
    try {
      & robocopy $emptyDir $Path /MIR /NFL /NDL /NJH /NJS /NP /R:2 /W:1 | Out-Null
      if ($LASTEXITCODE -gt 7) {
        throw "[build-windows-x64] robocopy cleanup failed for $Path (exit $LASTEXITCODE)"
      }
    } finally {
...
if (Test-Path $winUnpackedDir) {
  $canonicalWinUnpackedDir = Join-Path $canonicalOutputDir 'win-unpacked'
  & robocopy $winUnpackedDir $canonicalWinUnpackedDir /MIR /NFL /NDL /NJH /NJS /NP
  if ($LASTEXITCODE -gt 7) {
    throw "[build-windows-x64] robocopy failed while copying win-unpacked (exit $LASTEXITCODE)"
  }
