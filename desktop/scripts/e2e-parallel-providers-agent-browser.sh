...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/autoabm-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-autoabm-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-autoabm-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/autoabm" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'autoabm'), { recursive: true })
writeFileSync(join(configDir, 'autoabm', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('autoabm-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('autoabm-session-runtime', JSON.stringify({
