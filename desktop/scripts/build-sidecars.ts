...
const kernelSourceDir = path.join(repoRoot, 'packages', 'abm-kernel')
const bundledKernelDir = path.join(desktopRoot, 'src-tauri', 'resources', 'abm-kernel')
const bundledKernelPythonVersion = process.env.ABM_KERNEL_PYTHON_VERSION || '3.13'
...
async function syncAbmKernelResources() {
  console.log('[build-sidecars] syncing ABM kernel resources...')
  await rm(bundledKernelDir, { recursive: true, force: true })
  await mkdir(bundledKernelDir, { recursive: true })
  for (const entry of ['pyproject.toml', 'uv.lock', '.python-version', 'README.md', 'src']) {
    await cp(path.join(kernelSourceDir, entry), path.join(bundledKernelDir, entry), { recursive: true })
  }
  await syncAbmKernelRuntime()
}

async function syncAbmKernelRuntime() {
  const pythonInstallDir = path.join(bundledKernelDir, '.python')
  await runProcess('uv', [
    'python',
    'install',
    bundledKernelPythonVersion,
    '--install-dir',
    pythonInstallDir,
    '--no-bin',
    '--no-registry',
  ...
  const pythonExecutable = await findPythonExecutable(pythonInstallDir)
  if (!pythonExecutable) {
    throw new Error(`[build-sidecars] No bundled Python executable found under ${pythonInstallDir}`)
  }
...
async function findPythonExecutable(rootDir: string): Promise<string | null> {
...
async function runProcess(command: string, args: string[], cwd: string) {
  ...
  if (exitCode !== 0) {
    throw new Error(`[build-sidecars] ${command} failed with exit ${exitCode}`)
  }
}
