...
fn server_state_path() -> Option<PathBuf> {
    // Lives next to autoabm/settings.json (CLAUDE_CONFIG_DIR or ~/.claude) so
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show AutoABM")
        .separator()
        .text(TRAY_QUIT_ID, "Quit AutoABM")
        .build()?;
...
/// 解析 autoabm/settings.json 里的 h5Access.fixedPort。范围必须与
...
fn read_h5_fixed_port() -> Option<u16> {
    let path = claude_config_dir()?.join("autoabm").join("settings.json");
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 AutoABM").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "AutoABM")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
