...
export function ActiveSession() {
  ...
  const showAbmPanel = useAbmStore((state) =>
    activeTabId && isSessionTabState(activeTabId, activeTabType) && !isMemberSession && !isMobileLayout
      ? state.panelOpen
      : false,
  )
  const showRightPanel = showAbmPanel || showWorkbench
  ...
  const openCaseLibrary = (caseId?: string) => {
  ...
  const centeredHeroComposer = isEmpty && !isMemberSession && !showRightPanel
      ...
      <div data-testid="active-session-content-row" className="flex min-h-0 min-w-0 flex-1">
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              <div className="flex max-w-md flex-col items-center text-center">
                {isMemberSession ? (
                  ...
                  <>
                    <BrandMark size="xl" className="mb-6" />
                    <h1 className={`${compactEmptyHero ? 'mb-1 text-lg' : 'mb-2 text-xl'} font-semibold tracking-tight text-[var(--color-text-primary)]`} style={{ fontFamily: 'var(--font-headline)' }}>
                      {t('empty.title')}
                    </h1>
                    <p className={`mx-auto max-w-sm text-sm leading-6 text-[var(--color-text-secondary)]`} style={{ fontFamily: 'var(--font-body)' }}>
                      {t('empty.subtitle')}
                    </p>
                  </>
                )}
              ...
              {centeredHeroComposer && (
                <div className={`flex shrink-0 flex-col items-center px-8 ${compactEmptyHero ? 'pb-4' : 'pb-8'}`}>
            ...
            <>
              {!isMemberSession && !isMobileLayout && (
                <div
                  className={
                    showRightPanel
                      ? 'flex w-full justify-center border-b border-[var(--color-border)]/70 px-4 py-3'
                      : 'w-full border-b border-outline-variant/10 px-4 py-3'
                  }
                >
                  <div className={showRightPanel ? 'w-full max-w-[760px] min-w-0' : 'mx-auto w-full max-w-[860px] min-w-0'}>
        ...
        {showAbmPanel ? (
          <>
            ...
            <aside
              data-testid="abm-side-panel"
              className="flex h-full shrink-0 flex-col border-l border-[var(--color-border)] bg-[var(--color-surface)]"
              style={{ width: `clamp(48%, ${rightPanelWidth}px, 52%)`, maxWidth: '52%', minWidth: '48%' }}
            >
              <AbmWorkbench embedded workDir={getSessionTerminalCwd(session)} sessionId={activeTabId} />
            </aside>
          </>
        ) : showWorkbench ? (
          <>
