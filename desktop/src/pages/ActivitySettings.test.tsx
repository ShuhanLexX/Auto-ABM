...
describe('ActivitySettings', () => {
  ...
  it('renders summary metrics and a GitHub-style trailing heatmap without future days', async () => {
    ...
    expect(screen.getByRole('link', { name: 'github.com/SocialAI-X/Auto-ABM' })).toHaveAttribute(
      'href',
      'https://github.com/SocialAI-X/Auto-ABM',
    )
  ...
  it('handles avatar upload, fallback, removal, save failure, and cancel reset', async () => {
    ...
    expect(screen.getByText('AutoABM')).toBeInTheDocument()
    expect(screen.queryByAltText('AutoABM avatar')).not.toBeInTheDocument()
  })
