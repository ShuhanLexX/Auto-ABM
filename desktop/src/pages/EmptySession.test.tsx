...
vi.mock('../abm/components/AbmModeSelector', () => ({
  AbmModeSelector: ({ compact }: { compact?: boolean }) => (
    <button type="button" data-testid="abm-mode-selector" data-compact={compact ? 'true' : 'false'}>
      研究
    </button>
  ),
}))

vi.mock('../abm/AbmWorkbench', () => ({
  AbmWorkbench: ({ embedded, workDir }: { embedded?: boolean; workDir?: string }) => (
    <div data-testid="mock-empty-abm-workbench" data-embedded={embedded ? 'true' : 'false'} data-work-dir={workDir ?? ''} />
  ),
}))
...
import { useWorkspacePanelStore, WORKSPACE_PANEL_DEFAULT_WIDTH } from '../stores/workspacePanelStore'
...
describe('EmptySession', () => {
  ...
  beforeEach(() => {
    ...
    useWorkspacePanelStore.setState({ width: WORKSPACE_PANEL_DEFAULT_WIDTH })
  ...
  it('renders the ABM workbench panel from the empty state even without an active simulation', async () => {
    useAbmStore.setState({ panelOpen: true, activeSimId: null })

    render(<EmptySession />)

    await waitFor(() => {
      const panel = screen.getByTestId('empty-abm-workbench-panel')
      expect(within(panel).getByTestId('mock-empty-abm-workbench')).toHaveAttribute('data-embedded', 'true')
    })
  })

  it('lets the empty-state ABM workbench use the shared resizable width', async () => {
    useAbmStore.setState({ panelOpen: true, activeSimId: null })
    useWorkspacePanelStore.getState().setWidth(940)

    render(<EmptySession />)

    await waitFor(() => {
      expect(screen.getByTestId('empty-abm-workbench-resize-handle')).toBeInTheDocument()
    })
    expect(screen.getByTestId('empty-abm-workbench-panel').getAttribute('style')).toContain('clamp(48vw, 940px, 52vw)')
  })
  ...
  it('uses compact composer controls on phone-sized H5 browsers', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('abm-mode-selector')).toHaveAttribute('data-compact', 'true')
    })
    expect(screen.getByTestId('model-selector')).toHaveAttribute('data-compact', 'true')
    expect(screen.getByRole('button', { name: 'Run' })).toHaveClass('h-9', 'w-9')
    expect(screen.getByTestId('empty-session-composer-shell')).toHaveClass('px-3')
    expect(screen.getByTestId('empty-session-composer-panel')).toHaveClass('moon-composer')
  })
  ...
  it('integrates repository launch controls into the desktop composer panel', async () => {
    ...
    expect(panel).toHaveClass('moon-composer')
    expect(panel).not.toHaveClass('rounded-b-none')

    fireEvent.click(screen.getByRole('button', { name: 'Pick project', hidden: true }))

    const branchButton = await screen.findByRole('button', { name: 'Select branch: main', hidden: true })
  ...
  it('uses native desktop file paths for draft attachments', async () => {
    ...
    fireEvent.click(screen.getByText('Add data or papers'))
  ...
  it('inserts current research simulations from the empty-session @ menu', async () => {
    useAbmStore.setState({ activeProjectId: 'project-1' })
    mocks.listSimulations.mockResolvedValueOnce({
      simulations: [
        {
          id: 'sim-segregation',
          projectId: 'project-1',
    ...
    fireEvent.change(input, {
      target: {
        value: '@schelling 请解释机制。',
        selectionStart: '@schelling'.length,
      },
    })

    expect(await screen.findByText('当前研究问题的仿真')).toBeInTheDocument()
    await waitFor(() => {
      expect(mocks.listSimulations).toHaveBeenCalledWith('project-1')
    })
    fireEvent.click(await screen.findByText('谢林隔离模型'))

    await waitFor(() => {
      expect(input.value).toBe('@Simulation 谢林隔离模型 (id=sim-segregation) 请解释机制。')
    })
    expect(mocks.listSimulations).toHaveBeenCalledWith('project-1')
  })
  ...
  it('keeps the repository launch context on one row and truncates long branch names', async () => {
    ...
    const branchButton = await screen.findByRole('button', { name: new RegExp(`Select branch: ${longBranch}`), hidden: true })
  ...
  it('keeps current worktree selectable when the fallback branch is checked out elsewhere', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Select branch: main', hidden: true })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: /Select worktree mode: Current worktree/, hidden: true })).toBeInTheDocument()
    })

    const launchControls = screen.getByRole('button', { name: 'Select branch: main', hidden: true }).closest('[aria-hidden="true"]')
    expect(launchControls).toHaveTextContent('Selected branch is already checked out in another worktree')

    fireEvent.click(screen.getByRole('button', { name: /Select worktree mode: Current worktree/, hidden: true }))
