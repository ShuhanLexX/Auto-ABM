...
vi.mock('../abm/AbmWorkbench', () => ({
  AbmWorkbench: ({ embedded, workDir }: { embedded?: boolean; workDir?: string }) => (
    <div data-testid="mock-abm-workbench" data-embedded={embedded ? 'true' : 'false'} data-work-dir={workDir ?? ''} />
  ),
}))
...
vi.mock('../abm/api/abmClient', () => ({
  abmClient: {
    ...
    createProject: vi.fn(async () => ({
      id: 'project-created',
      name: '案例研究问题',
      researchQuestion: '案例研究问题',
      rootPath: '/tmp/project-created',
      createdAt: '2026-07-01T00:00:00.000Z',
      updatedAt: '2026-07-01T00:00:00.000Z',
    })),
...
describe('ActiveSession task polling', () => {
  ...
  it('renders the ABM workbench beside the chat when an active simulation is open', () => {
    const sessionId = 'abm-session'
    ...
    useTabStore.setState({
      tabs: [{ sessionId, title: 'ABM Session', type: 'session', status: 'idle' }],
      activeTabId: sessionId,
    })
    useChatStore.setState({
      sessions: {
    ...
    useWorkspacePanelStore.getState().openPanel(sessionId)
    useAbmStore.setState({ activeSimId: 'sim-1', panelOpen: true })

    render(<ActiveSession />)

    const abmPanel = screen.getByTestId('abm-side-panel')
    expect(screen.queryByTestId('workbench-panel')).toBeNull()
    expect(within(abmPanel).getByTestId('mock-abm-workbench')).toHaveAttribute('data-embedded', 'true')
    expect(screen.getByTestId('message-list')).toHaveAttribute('data-compact', 'true')
  })

  it('renders the ABM workbench even when no simulation is currently active', () => {
    const sessionId = 'abm-empty-session'
    ...
    useAbmStore.setState({ activeSimId: null, panelOpen: true })

    render(<ActiveSession />)

    expect(screen.getByTestId('abm-side-panel')).toBeInTheDocument()
    expect(screen.getByTestId('mock-abm-workbench')).toHaveAttribute('data-embedded', 'true')
  })

  it('shows research assets below the centered composer on a new research session', async () => {
    const sessionId = 'research-assets-empty-session'
    ...
    useTabStore.setState({
      tabs: [{ sessionId, title: 'New Research Session', type: 'session', status: 'idle' }],
      activeTabId: sessionId,
    })
    useChatStore.setState({
      sessions: {
    ...
    expect(screen.getByTestId('chat-input')).toHaveAttribute('data-variant', 'hero')
    expect(screen.getByTestId('research-asset-shelf')).toBeInTheDocument()
    expect(screen.getByText('最近仿真')).toBeInTheDocument()
    expect(screen.getByText('案例库精选')).toBeInTheDocument()
    await waitFor(() => {
      expect(screen.getByText('Schelling 城市隔离模型')).toBeInTheDocument()
    })
  })
  ...
  it('keeps the docked terminal usable on a new empty session', () => {
    ...
    expect(screen.getByTestId('empty-session-hero')).toHaveClass('pb-4')
