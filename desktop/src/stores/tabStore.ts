...
export const ABM_TAB_ID = '__abm__'
export const TERMINAL_TAB_PREFIX = '__terminal__'
export const TRACE_TAB_PREFIX = '__trace__'
export const WORKBENCH_TAB_PREFIX = '__workbench__'

export type TabType = 'session' | 'settings' | 'scheduled' | 'terminal' | 'trace' | 'traces' | 'workbench' | 'abm'
...
type TabStore = {
  ...
  openAbmTab: (title?: string) => string
...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  openAbmTab: (title = '仿真工作台') => {
    const { tabs } = get()
    const existing = tabs.find((tab) => tab.sessionId === ABM_TAB_ID)
    if (existing) {
      set({
        tabs: tabs.map((tab) => (
          tab.sessionId === ABM_TAB_ID ? { ...tab, title, type: 'abm' } : tab
        )),
        activeTabId: ABM_TAB_ID,
      })
    } else {
      set({
        tabs: [...tabs, { sessionId: ABM_TAB_ID, title, type: 'abm', status: 'idle' }],
        activeTabId: ABM_TAB_ID,
      })
    }
    get().saveTabs()
  ...
  saveTabs: () => {
    const { tabs, activeTabId } = get()
    const persistableTabs = tabs.filter((tab) => tab.type !== 'terminal' && tab.type !== 'workbench' && tab.type !== 'abm')
