...
export const useSessionStore = create<SessionStore>((set, get) => ({
  ...
  deleteSession: async (id: string) => {
    ...
    clearAbmWorkbenchState()
    await deleteAllAbmProjectsIfNoLocalSessionsRemain([id])
    set((s) => ({
  ...
  deleteSessions: async (ids: string[]) => {
    ...
    if (result.successes.length > 0) clearAbmWorkbenchState()
    await deleteAllAbmProjectsIfNoLocalSessionsRemain(result.successes)
    set((s) => ({
...
function clearAbmWorkbenchState(): void {
  const abm = useAbmStore.getState()
  abm.reset()
  abm.setActiveProject(null)
  abm.setActiveSim(null)
  abm.closePanel()
  clearAbmLocalStorageKeys()
}

async function deleteAllAbmProjectsIfNoLocalSessionsRemain(deletedIds: string[]): Promise<void> {
  if (deletedIds.length === 0) return
  const deleted = new Set(deletedIds)
  const remaining = useSessionStore.getState().sessions.filter((session) => !deleted.has(session.id))
  if (remaining.length > 0) return
  try {
    await abmClient.deleteAllProjects()
  } catch (error) {
    console.warn('[sessionStore] Failed to clear remaining ABM projects after deleting all sessions', error)
  }
}

function clearAbmLocalStorageKeys(): void {
  if (typeof localStorage === 'undefined') return
  try {
    for (let index = localStorage.length - 1; index >= 0; index -= 1) {
      const key = localStorage.key(index)
      if (!key) continue
      if (key === 'autoabm-default-project' || key.startsWith('autoabm-default-project:') || key.startsWith('autoabm-abm-default-sim')) {
