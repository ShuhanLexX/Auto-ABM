...
import { buildAbmContext, formatAbmReferencePrompt, getAbmLanguageInstruction, getAbmModeInstruction } from '../abm/composer/abmReferences'
import { useSettingsStore } from './settingsStore'
import { useAbmStore } from '../abm/stores/abmStore'
import { getAbmText } from '../abm/i18n'
...
import { parseAbmCard, type AbmCard } from '../abm/chat/abmCard'
...
function readSimIdFromToolInput(input: unknown): string | null {
  if (!isRecord(input)) return null
  return typeof input.simId === 'string' && input.simId.trim() ? input.simId.trim() : null
}

function readSimIdFromEditModelResult(content: unknown): string | null {
  const text = toolResultText(content)
  const explicit = text.match(/^simId:\s*([0-9a-f-]{8,})\s*$/im)
  if (explicit?.[1]) return explicit[1]
  const fallback = text.match(/^Model\s+([0-9a-f-]{8,})\s+/im)
...
function abmCardToUiMessage(card: AbmCard, timestamp: number): UIMessage {
  if (card.kind === 'proposal_batch') {
    return { id: nextId(), type: 'abm_proposal_batch', proposals: card.proposals, timestamp }
  }
  if (card.kind === 'result_canvas') {
    const activeRunId = useAbmStore.getState().activeRunId
    const runId = card.runId ?? activeRunId
    if (runId) {
    ...
    const metricText = card.metrics.join(', ')
    return {
      id: nextId(),
      type: 'system',
      content: runId
        ? (card.note ?? getAbmText('chat.resultCanvasAdded', { metrics: metricText }))
        : getAbmText('chat.resultCanvasNoRun', { metrics: metricText }),
      timestamp,
    }
  }
  if (card.kind === 'workbench') {
    const abm = useAbmStore.getState()
    if (card.simId) abm.setActiveSim(card.simId)
    if (card.runId) abm.setActiveRun(card.runId)
    if (card.action === 'close') {
      abm.closePanel()
    } else if (card.view) {
      abm.requestView(card.view)
    } else {
      abm.openPanel()
    }
    const viewLabels: Record<string, string> = {
      run: getAbmText('workbench.view.run'),
      results: getAbmText('workbench.view.results'),
      agents: getAbmText('workbench.view.agents'),
      model: getAbmText('workbench.view.model'),
      odd: getAbmText('workbench.view.odd'),
      simulations: getAbmText('workbench.view.simulations'),
    }
    const viewSuffix = card.view ? getAbmText('chat.workbenchViewSuffix', { view: viewLabels[card.view] ?? card.view }) : ''
    return {
      id: nextId(),
      type: 'system',
      content:
        card.note ??
        (card.action === 'close'
          ? getAbmText('chat.workbenchClosed')
          : getAbmText('chat.workbenchOpened', { view: viewSuffix })),
      timestamp,
    }
  }
  if (card.kind === 'experiment_view') {
    const abm = useAbmStore.getState()
    abm.setExperimentView(card.simId, card.view)
    abm.requestView('results')
    return {
      id: nextId(),
      type: 'system',
      content: card.note ?? getAbmText('chat.experimentViewCreated', { title: card.view.title }),
      timestamp,
    }
  }
  if (card.kind === 'attribution') {
    return {
      ...
      actualDelta: card.actualDelta,
      attributedNet: card.attributedNet,
  ...
  if (card.kind === 'counterfactual') {
    return {
      ...
      baseRunId: card.baseRunId,
      runId: card.runId,
      changed: card.changed,
      seed: card.seed,
      steps: card.steps,
      status: card.status,
      divergenceTick: card.divergenceTick,
      metrics: card.metrics,
      ...(card.note !== undefined ? { note: card.note } : {}),
      timestamp,
    }
  ...
  return {
    ...
    speculative: card.speculative,
    ...(card.runId ? { runId: card.runId } : {}),
    ...(card.from !== undefined ? { from: card.from } : {}),
    ...(card.to !== undefined ? { to: card.to } : {}),
    timestamp,
  }
}
...
function appendThinkingTextUpdate(
  session: PerSessionState,
  text: string,
): Pick<PerSessionState, 'messages' | 'chatState' | 'activeThinkingId' | 'streamingText' | 'streamingResponseChars'> & { elapsedSeconds?: number } {
  // Reset the elapsed counter when this is the start of a new thinking step
  // (i.e. we are entering `thinking` from another phase), so the indicator
  // reflects the current step's duration rather than the whole turn's.
  const enteringThinking = session.chatState !== 'thinking' ? { elapsedSeconds: 0 } : {}
  ...
  if (last && last.type === 'thinking') {
    ...
    return {
      ...
      streamingText: '',
      streamingResponseChars: session.streamingResponseChars + text.length,
      ...enteringThinking,
    }
  }
  const id = nextId()
...
function buildAgentCompletionNotification(
  ...
  return {
    title: localizedStoreText('AutoABM completed a response', 'AutoABM 已完成回复'),
...
function localizedStoreText(en: string, zh: string): string {
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  sendMessage: (sessionId, content, attachments, options) => {
    ...
    const cleanContent = stripInjectedAbmPromptPrefix(content)
    const outboundContent = withAbmContext(cleanContent)
    const userFacingContent =
      hideDisplayContent
        ? ''
        : options?.displayContent?.trim() || cleanContent.trim()
    const modelFacingContent = buildModelContent(outboundContent, attachments)
    ...
    wsManager.send(sessionId, { type: 'user_message', content: outboundContent, attachments })
  },
  ...
  queueUserMessage: (sessionId, message) => {
    ...
    set((state) => {
      const session = state.sessions[sessionId] ?? createDefaultSessionState()
      const displayContent = stripInjectedAbmPromptPrefix(message.displayContent).trim()
      return {
        sessions: {
          ...state.sessions,
          [sessionId]: {
  ...
  updateQueuedUserMessage: (sessionId, messageId, content) => {
    const nextContent = stripInjectedAbmPromptPrefix(content).trim()
    ...
    switch (msg.type) {
      ...
      case 'status':
        update((session) => {
          ...
          const nextChatState = preserveStreamingTurn ? 'streaming' : msg.state
          const nextStatusVerb = msg.state === 'idle'
            ? ''
            : msg.verb && msg.verb !== 'Thinking'
              ? msg.verb
              : ''
          // A new server phase (state or status verb) starts a new step; reset the
          // elapsed counter so the indicator times the current step, not the turn.
          const phaseChanged = nextChatState !== session.chatState || nextStatusVerb !== session.statusVerb
          return {
            chatState: nextChatState,
            statusVerb: nextStatusVerb,
            ...(phaseChanged && msg.state !== 'idle' ? { elapsedSeconds: 0 } : {}),
      ...
      case 'content_start': {
        ...
        if (msg.blockType === 'text') {
          update((s) => ({
            ...
            apiRetry: null,
            streamingFallback: null,
            ...(s.chatState !== 'streaming' ? { elapsedSeconds: 0 } : {}),
          }))
          ...
          update((s) => ({
            ...
            apiRetry: null,
            streamingFallback: null,
            ...(s.chatState !== 'tool_executing' ? { elapsedSeconds: 0 } : {}),
          }))
        }
        break
      }
        ...
        if (msg.text !== undefined) {
          ...
          if (!flushTimerBySession.has(sessionId)) {
            const timer = setTimeout(() => {
              ...
              flushTimerBySession.delete(sessionId)
              update((s) => {
                if (s.chatState === 'thinking') {
                  return appendThinkingTextUpdate(s, text)
                }
                return {
      ...
      case 'thinking': {
        const pending = consumePendingDelta(sessionId)
        update((s) => {
          const pendingText = `${s.streamingText}${pending}`
          const state = pendingText.trim()
            ? {
                ...s,
                messages: appendAssistantTextMessage(s.messages, pendingText, Date.now()),
                streamingText: '',
                streamingResponseChars: s.streamingResponseChars + pending.length,
              }
            : pending
              ? {
                  ...s,
              ...
              : s
          return { ...state, ...appendThinkingTextUpdate(state, msg.text) }
        })
        break
      }
      ...
      case 'tool_result': {
        ...
        // ABM tools encode rich cards (proposal batch / explanation) in their
        // tool_result content; render those instead of the raw result block.
        const abmCard = msg.isError ? null : parseAbmCard(msg.content)
        update((s) => {
          let messages: UIMessage[] = [...s.messages, abmCard
            ? abmCardToUiMessage(abmCard, now)
            : {
          ...
          return {
            ...
            chatState: 'thinking',
            activeThinkingId: null,
            ...(s.chatState !== 'thinking' ? { elapsedSeconds: 0 } : {}),
          }
        })
        if (consumePendingTaskToolUseId(sessionId, msg.toolUseId)) {
          useCLITaskStore.getState().refreshTasks(sessionId)
        }
        const relatedTool = findToolUseMessage(get().sessions[sessionId]?.messages ?? [], msg.toolUseId)
        if (!msg.isError && relatedTool?.toolName === 'abm_edit_model') {
          const inputSimId = readSimIdFromToolInput(relatedTool.input)
          const resultSimId = readSimIdFromEditModelResult(msg.content)
          const abmStore = useAbmStore.getState()
          if (resultSimId && resultSimId !== inputSimId) abmStore.setActiveSim(resultSimId)
          abmStore.markSimulationChanged(resultSimId ?? inputSimId)
        }
        break
      }
      ...
      case 'permission_request':
        notifyDesktop({
          ...
          title: localizedStoreText('AutoABM needs your approval', 'AutoABM 需要你的确认'),
          body: msg.toolName
            ? localizedStoreText(`${msg.toolName} wants to run and is waiting for approval.`, `${msg.toolName} 请求执行，正在等待允许。`)
            : localizedStoreText('A tool request is waiting for approval.', '有一个工具请求正在等待允许。'),
      ...
      case 'computer_use_permission_request':
        notifyDesktop({
          ...
          requestAttention: true,
          title: localizedStoreText('AutoABM needs your approval', 'AutoABM 需要你的确认'),
          body: msg.request.reason || localizedStoreText('Computer Use is waiting for approval.', 'Computer Use 正在等待允许。'),
          target: { type: 'session', sessionId },
        })
        update(() => ({
          pendingComputerUsePermission: {
...
function withAbmContext(content: string): string {
  const { activeSimId, activeProjectId, mode } = useAbmStore.getState()
  const { locale, responseLanguage } = useSettingsStore.getState()
  const isAbmRequest = looksLikeAbmRequest(content)
  if (!activeSimId && !isAbmRequest) return content
  const bindActiveSimulation = activeSimId && !looksLikeNewSimulationRequest(content)
  const parts: string[] = []
  const language = getAbmLanguageInstruction(locale, responseLanguage)
  if (language) parts.push(language)
  parts.push(getAbmModeInstruction(mode))
  const abmPrefix = formatAbmReferencePrompt(buildAbmContext({
    activeSimId: bindActiveSimulation ? activeSimId : null,
    activeProjectId,
  }))
  if (abmPrefix) parts.push(abmPrefix)
  if (parts.length === 0) return content
  const trimmed = content.trim()
  return trimmed ? `${parts.join('\n\n')}\n\n${trimmed}` : parts.join('\n\n')
}

export function stripInjectedAbmPromptPrefix(content: string): string {
  let next = content.replace(/\r\n?/g, '\n').trimStart()
  let changed = true
  while (changed) {
    const before = next
    next = next.replace(
      /^\s*Language:\s*(?:Always respond in|Preferred response language:)[\s\S]*?Keep code identifiers and technical parameter ids in their original form\.\s*/i,
      '',
    )
    next = next.replace(
      /^\s*ABM mode:\s*(?:Dialogue|Autonomous exploration|Research)\.[\s\S]*?(?:\n{2,}|$)/i,
      '',
    )
    next = next.replace(
      /^[ \t]*ABM context:[ \t]*(?:\n(?:- [^\n]*|\(bound[^\n]*\))[ \t]*)+(?:\n{2,}|$)/i,
...
function looksLikeAbmRequest(content: string): boolean {
  return /ABM|AutoABM|simulation|simulations|agent[-\s]?based|multi[-\s]?agent|experiment|mechanism|wildfire|fire\s+spread|grid[-\s]?based|network\s+model|opinion\s+dynamics|public\s+goods|segregation|epidemic|infection|diffusion|仿真|模拟|智能体|模型|机制|参数|实验|Trace|ODD|结果图|画布|谣言|舆情|扩散|隔离|合作|山火|交通|涌现|复杂系统/i.test(content)
}

function looksLikeNewSimulationRequest(content: string): boolean {
  return /\b(create|build|generate|design|make|start|new)\b[\s\S]{0,80}\b(simulation|model|experiment|abm)\b|新建|创建|生成|设计|构建|做一个|建一个|仿真实验|新的仿真/i.test(content)
}
...
function extractVisibleUserMessage(rawContent: string): {
  content: string
  attachments?: UIAttachment[]
  modelContent?: string
} {
  const sanitized = stripGeneratedImageMetadataLines(rawContent) || rawContent.trim()
  const visibleContent = stripInjectedAbmPromptPrefix(sanitized)
  const parsed = extractLeadingFileReferences(visibleContent)
  const visibleModelContent = (parsed.modelContent ?? parsed.content).trim()
  const rawModelContent = sanitized.trim()
  const modelContent =
    rawModelContent && rawModelContent !== visibleModelContent
      ? rawModelContent
      : parsed.modelContent
  return {
...
export function appendReplayedUserMessage(
  ...
  const parsed = extractVisibleUserMessage(content)
  const displayContent = parsed.content.trim()
  if (!displayContent) return messages

  const modelContent = parsed.modelContent ?? displayContent
...
export function mapHistoryMessagesToUiMessages(
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'user' && typeof msg.content === 'string') {
      ...
      const parsed = extractVisibleUserMessage(msg.content)
      uiMessages.push({
        id: msg.id || nextId(),
        type: 'user_text',
        content: parsed.content,
    ...
    if ((msg.type === 'user' || msg.type === 'tool_result') && Array.isArray(msg.content)) {
      ...
      for (const block of msg.content as UserHistoryBlock[]) {
        if (block.type === 'text' && block.text && isTeammateMessage(block.text)) {
          ...
          // Rebuild rich ABM cards (proposal batch / explanation) from the
          // transcript, mirroring the live tool_result path — otherwise the
          // interactive cards silently disappear after a history reload.
          const abmCard = block.is_error ? null : parseAbmCard(block.content)
          // Only pure display cards are rebuilt; side-effecting cards
          // (result_canvas / workbench / experiment_view) must not re-trigger
          // panel or chart mutations when history is re-hydrated.
          if (
            abmCard &&
            (abmCard.kind === 'proposal_batch' ||
              abmCard.kind === 'explanation' ||
              abmCard.kind === 'attribution' ||
              abmCard.kind === 'counterfactual')
          ) {
            uiMessages.push(abmCardToUiMessage(abmCard, timestamp))
          } else {
            uiMessages.push({
              id: nextId(),
              type: 'tool_result',
      ...
      if (visibleTextParts.length > 0 || attachments.length > 0) {
        ...
        const displayText = stripInjectedAbmPromptPrefix(visibleText)
        ...
        const parsed = extractLeadingFileReferences(displayText)
        const userContent = visualSelectionDisplay ? '' : parsed.content
        const visibleModelContent = parsed.modelContent ?? displayText
        const modelContent = visualSelectionDisplay || modelText !== visibleModelContent ? modelText : parsed.modelContent
