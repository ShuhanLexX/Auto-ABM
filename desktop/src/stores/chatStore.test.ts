...
import {
  ...
  stripInjectedAbmPromptPrefix,
  stripGeneratedImageMetadataLines,
  type PerSessionState,
  useChatStore,
} from './chatStore'
import { getAbmLanguageInstruction } from '../abm/composer/abmReferences'
...
describe('stripInjectedAbmPromptPrefix', () => {
  it('removes hidden ABM language, mode, and context prefixes from visible user text', () => {
    const hiddenPrompt = [
      getAbmLanguageInstruction('en', ''),
      'ABM mode: Research. Case-library examples are display-only examples for users. Do not inspect, cite, import, or reuse a case/template/current simulation as the starting point for a new model unless the user explicitly selected that case or asked to modify the active simulation.',
      'ABM context:\n- @Simulation Active model (id=sim-1)\n(bound to the active Simulation by default; @-reference to override)',
      'Create a wildfire simulation in English.',
    ].join('\n\n')

    expect(stripInjectedAbmPromptPrefix(hiddenPrompt)).toBe('Create a wildfire simulation in English.')
  })
})

describe('appendReplayedUserMessage', () => {
  it('hides injected ABM prompt prefixes from replayed user bubbles', () => {
    const userPrompt = 'Build a single-point wildfire model.'
    const hiddenPrompt = [
      getAbmLanguageInstruction('en', ''),
      'ABM mode: Research. Clarify rough modeling needs before generating proposals.',
      userPrompt,
    ].join('\n\n')

    expect(appendReplayedUserMessage([], hiddenPrompt, 123)).toMatchObject([
      {
        type: 'user_text',
        content: userPrompt,
        modelContent: hiddenPrompt,
      },
    ])
  })
})
...
describe('chatStore history mapping', () => {
  beforeEach(() => {
    ...
    useSettingsStore.setState({ locale: 'zh', responseLanguage: '' })
  ...
  it('restores ABM-prefixed history without exposing hidden instructions', () => {
    const userPrompt = 'Create a multi-point wildfire simulation.'
    const hiddenPrompt = [
      getAbmLanguageInstruction('en', ''),
      'ABM mode: Autonomous exploration. Treat the user message as a research objective, not a single Q&A prompt.',
      'ABM context:\n- @Simulation sim-1 (id=sim-1)',
      userPrompt,
    ].join('\n\n')
  ...
  it('strips stale injected ABM language instructions before displaying or sending', () => {
    useSettingsStore.setState({ locale: 'en', responseLanguage: 'chinese' })
    useAbmStore.setState({ activeSimId: null, mode: 'autonomous' })
    useChatStore.setState({
      sessions: {
    ...
    const userPrompt = 'Single-point ignition grid wildfire simulation with rocks and trees.'
    const staleInstruction = [
      'Language: Always respond in chinese. All user-facing text — including AskUserQuestion questions, tab headers, option labels/descriptions, proposal summaries, and explanations — must use chinese.',
    ...
    useChatStore.getState().sendMessage(TEST_SESSION_ID, `${staleInstruction}\n\n${userPrompt}`)

    const message = useChatStore.getState().sessions[TEST_SESSION_ID]?.messages[0]
    expect(message).toMatchObject({
      type: 'user_text',
      content: userPrompt,
    })
    if (!message || message.type !== 'user_text') throw new Error('Expected the sent message to be a user_text message')
    expect(message.modelContent).toContain('Preferred response language: english')
    expect(message.modelContent).not.toContain('Always respond in chinese')
    expect(sendMock).toHaveBeenCalledWith(TEST_SESSION_ID, expect.objectContaining({
      content: expect.stringContaining('Preferred response language: english'),
    }))
    expect(sendMock).toHaveBeenCalledWith(TEST_SESSION_ID, expect.objectContaining({
      content: expect.not.stringContaining('Always respond in chinese'),
    }))
  })
  ...
  it('routes content_delta text into a thinking block while chatState is thinking', () => {
    vi.useFakeTimers()
    ...
    const session = useChatStore.getState().sessions[TEST_SESSION_ID]
    expect(session?.activeThinkingId).toBeTruthy()
    expect(session?.messages).toMatchObject([
      { type: 'thinking', content: '先梳理舆情演化' },
    ])
...
describe('chatStore ABM card mapping', () => {
  beforeEach(() => {
    useChatStore.setState({ sessions: { [TEST_SESSION_ID]: makeSession() } })
    useAbmStore.setState({
      activeRunId: 'run-1',
  ...
  function emitToolResult(content: unknown) {
    useChatStore.getState().handleServerMessage(TEST_SESSION_ID, {
      type: 'tool_use_complete',
      toolName: 'abm_tool',
      toolUseId: 'tool-1',
  ...
  it('maps a proposal_batch envelope to an abm_proposal_batch message', () => {
    const messages = emitToolResult(
      JSON.stringify({
        abmCard: 'proposal_batch',
        proposals: [
          { id: 'p1', mechanismSummary: 'spread', expectedMacro: 'S-curve' },
          { id: 'p2', mechanismSummary: 'decay', expectedMacro: 'decline' },
        ],
      }),
    )
    const last = messages.at(-1)
    expect(last?.type).toBe('abm_proposal_batch')
    if (last?.type === 'abm_proposal_batch') expect(last.proposals).toHaveLength(2)
  })

  it('maps an explanation envelope to an abm_explanation message', () => {
    const messages = emitToolResult(
      JSON.stringify({
        abmCard: 'explanation',
        text: 'peaked at 5',
        evidence: [{ tick: 5, metric: 'infected' }],
        speculative: true,
        runId: 'run-1',
      }),
    )
    const last = messages.at(-1)
    expect(last?.type).toBe('abm_explanation')
    if (last?.type === 'abm_explanation') {
      expect(last.speculative).toBe(true)
      expect(last.runId).toBe('run-1')
      expect(last.evidence).toHaveLength(1)
    }
  })

  it('maps an attribution envelope to an abm_attribution message', () => {
    const messages = emitToolResult(
      JSON.stringify({
        abmCard: 'attribution',
        runId: 'run-1',
        ...
        residual: 2,
        coverage: 0.92,
        contributions: [{ mechanism_id: 'spread', gains: 30, losses: 0, net: 30, agents: 28 }],
      }),
    )
    const last = messages.at(-1)
    expect(last?.type).toBe('abm_attribution')
    if (last?.type === 'abm_attribution') {
      expect(last.metric).toBe('infected')
      expect(last.coverage).toBeCloseTo(0.92)
      expect(last.contributions).toHaveLength(1)
    }
  })

  it('maps a counterfactual envelope to an abm_counterfactual message', () => {
    const messages = emitToolResult(
      JSON.stringify({
        ...
        status: 'completed',
        divergenceTick: 7,
        metrics: [{ metric: 'infected', baseFinal: 30, otherFinal: 48, finalDelta: 18, maxAbsDelta: 21, maxAbsDeltaTick: 33 }],
      }),
    )
    const last = messages.at(-1)
    expect(last?.type).toBe('abm_counterfactual')
    if (last?.type === 'abm_counterfactual') {
      expect(last.baseRunId).toBe('run-base')
      expect(last.divergenceTick).toBe(7)
      expect(last.metrics[0]?.finalDelta).toBe(18)
    }
  })

  it('maps a result_canvas envelope to a result chart request', () => {
    const messages = emitToolResult(
      JSON.stringify({
        abmCard: 'result_canvas',
        metrics: ['burned_rate', 'burning'],
        action: 'show',
        runId: 'run-1',
      }),
    )
    ...
    expect(useAbmStore.getState().resultCharts['run-1']?.metrics).toEqual(['burned_rate', 'burning'])
  })

  it('leaves an ordinary tool_result untouched', () => {
    const messages = emitToolResult('just plain output')
    const last = messages.at(-1)
    expect(last?.type).toBe('tool_result')
  })

  it('does not treat an errored ABM envelope as a card', () => {
    useChatStore.getState().handleServerMessage(TEST_SESSION_ID, {
      type: 'tool_use_complete',
      toolName: 'abm_tool',
      toolUseId: 'tool-err',
    ...
    useChatStore.getState().handleServerMessage(TEST_SESSION_ID, {
      type: 'tool_result',
      toolUseId: 'tool-err',
      content: JSON.stringify({ abmCard: 'explanation', text: 'x', evidence: [], speculative: false }),
      isError: true,
    })
    const last = useChatStore.getState().sessions[TEST_SESSION_ID]?.messages.at(-1)
    expect(last?.type).toBe('tool_result')
  })
})
