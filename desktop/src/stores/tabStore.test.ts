...
import { ABM_TAB_ID, SETTINGS_TAB_ID, useTabStore } from './tabStore'
...
describe('tabStore', () => {
  ...
  it('opens one ephemeral workbench tab per source session', () => {
    ...
    expect(localStorage.getItem('autoabm-open-tabs')).toBe(JSON.stringify({
      openTabs: [],
      activeTabId: null,
    }))
  })

  it('drops persisted ABM workbench tabs because the workbench now lives in the conversation panel', async () => {
    localStorage.setItem('autoabm-open-tabs', JSON.stringify({
      openTabs: [
        { sessionId: ABM_TAB_ID, title: 'Simulation', type: 'abm' },
        { sessionId: 'session-1', title: 'Research chat', type: 'session' },
      ],
      activeTabId: ABM_TAB_ID,
    }))
    vi.mocked(sessionsApi.list).mockResolvedValue({
      sessions: [{ id: 'session-1', title: 'Research chat' }],
    } as never)
