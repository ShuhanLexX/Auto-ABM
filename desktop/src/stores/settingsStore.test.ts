...
describe('settingsStore locale defaults', () => {
  ...
  it('keeps a stored locale override', async () => {
    window.localStorage.setItem('autoabm-locale', 'en')
  ...
  it('does not auto-write a Chinese response language from the UI locale', async () => {
    window.localStorage.setItem('autoabm-locale', 'zh')
    ...
    vi.doMock('../api/h5Access', () => ({
      h5AccessApi: {
        get: vi.fn().mockResolvedValue({
          settings: { enabled: false, tokenPreview: null, allowedOrigins: [], publicBaseUrl: null },
        }),
        enable: vi.fn(),
        disable: vi.fn(),
        regenerate: vi.fn(),
    ...
    expect(useSettingsStore.getState().responseLanguage).toBe('')
    expect(updateUser).not.toHaveBeenCalledWith({ language: 'chinese' })
  })
})
...
describe('settingsStore UI zoom', () => {
  ...
  it('hydrates from the app zoom storage key', async () => {
    window.localStorage.setItem('autoabm-app-zoom', '1.25')
  ...
  it('clamps UI zoom changes to the supported range', async () => {
    ...
    await vi.waitFor(() => {
      expect(window.localStorage.getItem('autoabm-app-zoom')).toBe('2')
    })
...
describe('settingsStore app mode', () => {
  ...
  it('hydrates app mode from the Electron desktop host', async () => {
    const getAppMode = vi.fn().mockResolvedValue({
      mode: 'portable',
      portableDir: 'C:\\autoabm\\CLAUDE_CONFIG_DIR',
      defaultPortableDir: 'C:\\autoabm\\CLAUDE_CONFIG_DIR',
    })
  ...
  it('hydrates app mode from an injected desktop host', async () => {
    const getAppMode = vi.fn().mockResolvedValue({
      mode: 'portable',
      portableDir: 'D:\\autoabm\\data',
      defaultPortableDir: 'D:\\autoabm\\data',
    })
    installElectronAppModeHost({ get: getAppMode })
  ...
  it('persists app mode through the Electron desktop host and marks restart required', async () => {
    ...
    expect(useSettingsStore.getState().appMode).toEqual({
      ...
      portableDir: 'C:\\autoabm\\CLAUDE_CONFIG_DIR',
      defaultPortableDir: 'C:\\autoabm\\CLAUDE_CONFIG_DIR',
      activeConfigDir: 'C:\\autoabm\\CLAUDE_CONFIG_DIR',
