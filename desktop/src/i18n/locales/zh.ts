...
export const zh: Record<TranslationKey, string> = {
  ...
  // ─── Sidebar ──────────────────────────────────────
  'sidebar.abmWorkbench': '仿真工作台',
  'sidebar.caseLibrary': '案例库',
  'sidebar.newSession': '新建研究会话',
  ...
  'sidebar.blankProjectDialogTitle': '新建仿真目录',
  'sidebar.blankProjectDialogBody': '将在默认 AutoABM 目录下创建文件夹，并在其中开始研究会话。',
  'sidebar.blankProjectNameLabel': '目录名称',
  'sidebar.blankProjectNamePlaceholder': '市场群体行为实验',
  'sidebar.createBlankProject': '创建项目',
  ...
  'sidebar.newSessionInProject': '在 {project} 中新建研究会话',
  ...
  'settings.terminal.description': '直接运行宿主机命令，用于安装插件、Skills、MCP 等扩展。桌面端已内置 autoabm；文档里的 claude <参数> 可替换成 autoabm <参数>，例如 autoabm plugin install ... 或 autoabm mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '只会清理：autoabm-open-tabs、autoabm-session-runtime、autoabm-theme、autoabm-locale 和 autoabm.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.autoabm/autoabm/settings.json — 直接编辑，保存时写入。',
  ...
  'settings.general.storageSystemDescription': '回到默认数据源。若启动环境设置了 AUTOABM_CONFIG_DIR，则仍会优先使用该环境变量指定的目录。',
  ...
  'settings.general.storagePortableDirPlaceholder': '选择一个用于保存 autoabm 数据的文件夹',
  ...
  'settings.general.storageEnvironmentHint': '当前目录由 AUTOABM_CONFIG_DIR 环境变量控制。应用内切换不会覆盖这个环境变量；如需回到系统目录，请先移除启动环境里的 AUTOABM_CONFIG_DIR。',
  'settings.general.storageEnvironmentSwitchBlocked': '当前由 AUTOABM_CONFIG_DIR 环境变量控制。请先移除启动环境里的 AUTOABM_CONFIG_DIR，再切回系统目录。',
  'settings.general.storageRestartHint': '已保存切换请求。请重启应用，让新的数据目录生效。',
  'settings.general.storageMoveHint': '切换目录不会自动搬迁旧数据。如果希望旧会话继续出现，请把原目录下的 projects、skills、plugins、autoabm 等数据复制到新目录。要做便携包，建议把目录放在应用旁边并一起压缩。',
  ...
  'settings.general.notificationsTestTitle': 'AutoABM 通知已启用',
  ...
  'settings.general.traceHintOn': '新会话会把精简请求、响应、状态事件写入本机 autoabm traces 目录。',
  ...
  'empty.subtitle': '自动化 Agent-Based Model 建模、实验与发现',
  'empty.placeholder': '描述你想研究的现象或系统…',
  'empty.addFiles': '添加数据或论文',
  'empty.slashCommands': '斜杠命令',
  'empty.quick.code': '生成仿真方案',
  'empty.quick.app': '复现已有模型',
  'empty.quick.components': '参数扫描',
  'empty.quick.theme': '机制归因',
  'empty.quick.landing': '解释运行区间',
  'empty.quick.docs': '从数据/论文建模',
  'empty.quick.assets': '导出复现包',
  'empty.quick.codePrompt': '我有一个初步研究选题。请先不要直接生成仿真方案；先用 4-7 个问题澄清研究现象、智能体、交互结构、空间/网络表示、数据证据、关键指标与实验目标。等我回答后，再给候选仿真方案：',
  'empty.quick.appPrompt': '帮我复现这个经典 ABM 模型（如 SIR、Schelling、谣言传播），并说明设定：',
  'empty.quick.componentsPrompt': '对以下参数做单参数扫描，比较关键指标随取值的变化：',
  'empty.quick.themePrompt': '结合 Trace 证据链，解释是哪些机制导致了这一结果：',
  'empty.quick.landingPrompt': '解释这段运行区间里指标的变化，并给出引用的 Trace 证据：',
  'empty.quick.docsPrompt': '根据这些数据/论文帮我构建一个 ABM 模型：',
  'empty.quick.assetsPrompt': '为这次运行导出可复现包（seed、参数、模型版本、内核版本）：',
  ...
  // ─── Chat Input ──────────────────────────────────────
  'chat.placeholder': '让 AutoABM 建模、运行、比较或解释仿真...',
  ...
  'computerUseApproval.hideWhileWorking': 'AutoABM 工作时会隐藏另外 {count} 个应用。',
  'computerUseApproval.hideWhileWorkingRestore': 'AutoABM 工作时会隐藏另外 {count} 个应用，结束后会自动恢复。',
  ...
  'computerUseApproval.tryAgainHint': '“稍后重试”会把控制权交还给 AutoABM，让它在 macOS 权限生效后重新调用 request_access。',

  // ─── Ask User Question ──────────────────────────────────────
  'question.needsInput': 'AutoABM 需要你的输入',
