...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('autoabm-chat-history', 'preserve')
    window.localStorage.setItem('autoabm-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('autoabm-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('autoabm-provider-config')).toBe('preserve')
  })
  ...
  it('keeps local repair successful when the server doctor endpoint is unavailable', async () => {
    ...
    expect(result.local.removedKeys).toContain('autoabm-theme')
