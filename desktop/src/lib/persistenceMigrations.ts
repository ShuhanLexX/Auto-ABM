...
export const DESKTOP_PERSISTENCE_VERSION_KEY = 'autoabm.persistence.schemaVersion'
...
const TAB_STORAGE_KEY = 'autoabm-open-tabs'
const SESSION_RUNTIME_STORAGE_KEY = 'autoabm-session-runtime'
const THEME_STORAGE_KEY = 'autoabm-theme'
const LEGACY_THEME_STORAGE_KEY = 'cc-haha-theme'
const LOCALE_STORAGE_KEY = 'autoabm-locale'
...
function migrateLegacyBrandingKeys(storage: StorageLike, report: DesktopMigrationReport): void {
  const legacyTheme = storage.getItem(LEGACY_THEME_STORAGE_KEY)
  if (legacyTheme !== null) {
    if (!storage.getItem(THEME_STORAGE_KEY) && THEME_MODES.includes(legacyTheme as typeof THEME_MODES[number])) {
      storage.setItem(THEME_STORAGE_KEY, legacyTheme)
      report.migratedKeys.push(THEME_STORAGE_KEY)
    }
    storage.removeItem(LEGACY_THEME_STORAGE_KEY)
    report.migratedKeys.push(LEGACY_THEME_STORAGE_KEY)
  }

  const rebrandDarkKey = 'autoabm.rebrand.ui-v2-dark'
  if (!storage.getItem(rebrandDarkKey)) {
    storage.setItem(THEME_STORAGE_KEY, 'dark')
    storage.setItem(rebrandDarkKey, '1')
    report.migratedKeys.push(rebrandDarkKey)
  }
}
...
export function runDesktopPersistenceMigrations(storage: StorageLike | null = getDefaultStorage()): DesktopMigrationReport {
  ...
  runMigrationStep(report, THEME_STORAGE_KEY, () => migrateLegacyBrandingKeys(storage, report))
