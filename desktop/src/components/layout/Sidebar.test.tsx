...
vi.mock('../../i18n', () => ({
  useTranslation: () => (key: string, params?: Record<string, string | number>) => {
    const translations: Record<string, string> = {
      ...
      'sidebar.blankProjectDialogTitle': 'New simulation directory',
      'sidebar.blankProjectDialogBody': 'Create a named AutoABM research folder and start the session there.',
      'sidebar.blankProjectNameLabel': 'Directory name',
      'sidebar.blankProjectNamePlaceholder': 'Market behavior experiment',
      'sidebar.createBlankProject': 'Create project',
...
vi.mock('../shared/DirectoryPicker', () => ({
  DirectoryPicker: ({ onChange }: { value: string; onChange: (path: string) => void }) => (
    <button type="button" onClick={() => onChange('/workspace/web-project')}>
      Pick /workspace/web-project
    </button>
  ),
}))
...
describe('Sidebar', () => {
  ...
  beforeEach(() => {
    ...
    sessionsApiMock.createResearchDirectory.mockReset()
    sessionsApiMock.createResearchDirectory.mockResolvedValue({
      name: 'Market Dynamics',
      workDir: '/home/test/AutoABM/Market Dynamics',
    })
  ...
  it('shows project header menus and starts a blank project session in a named directory', async () => {
    ...
    const dialog = screen.getByRole('dialog', { name: 'New simulation directory' })
    fireEvent.change(within(dialog).getByLabelText('Directory name'), { target: { value: 'Market Dynamics' } })
    ...
    await waitFor(() => {
      expect(sessionsApiMock.createResearchDirectory).toHaveBeenCalledWith('Market Dynamics')
      expect(createSession).toHaveBeenCalledWith('/home/test/AutoABM/Market Dynamics')
      expect(connectToSession).toHaveBeenCalledWith('session-blank-project')
    })
  })

  it('lets browser users create a session from an existing folder', async () => {
    createSession.mockResolvedValue('session-web-project')
    useSessionStore.setState({
      sessions: [
        makeSession('alpha-1', 'Alpha Session', '/workspace/alpha', new Date().toISOString()),
      ],
    ...
    fireEvent.click(within(dialog).getByRole('button', { name: 'Pick /workspace/web-project' }))
    ...
    await waitFor(() => {
      expect(createSession).toHaveBeenCalledWith('/workspace/web-project')
      expect(connectToSession).toHaveBeenCalledWith('session-web-project')
    })
  })
  ...
  it('does not restore a hidden Windows drive root when creating a child project session', async () => {
    ...
    useSessionStore.setState({
      sessions: [
        makeSession('child-1', 'Child Session', 'D:\\workspace\\code\\autoabm', now),
      ],
    })
    useTabStore.setState({
    ...
    await waitFor(() => {
      expect(createSession).toHaveBeenCalledWith('D:\\workspace\\code\\autoabm')
    })
