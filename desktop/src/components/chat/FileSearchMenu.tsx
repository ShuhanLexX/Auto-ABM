...
import { abmClient } from '../../abm/api/abmClient'
import { readModelId, readModelVersion } from '../../abm/modelIntrospection'
...
export const FileSearchMenu = forwardRef<FileSearchMenuHandle, Props>(({
  ...
  useEffect(() => {
    if (!includeAbmReferences || !activeProjectId) {
      setSimulations([])
      return
    }
    ...
    abmClient.listSimulations(activeProjectId)
      .then((result) => {
        if (!cancelled) setSimulations(result.simulations)
      })
      .catch(() => {
        if (!cancelled) setSimulations([])
      })
      .finally(() => {
        if (!cancelled) setLoadingSimulations(false)
      })

    return () => {
      cancelled = true
    }
  }, [activeProjectId, includeAbmReferences])

  const filteredSimulations = useMemo(() => {
    const normalized = filter.trim().toLowerCase()
    if (!normalized) return simulations
    return simulations.filter((simulation) => {
      const haystack = `${simulation.name} ${simulation.id} ${readModelId(simulation.config, simulation.name)}`.toLowerCase()
  ...
  return (
    ...
    >
      ...
      {includeAbmReferences ? (
        <div className="border-b border-[var(--color-border)] bg-[var(--color-surface-container-low)]/45 px-2 py-2">
          <div className="mb-1 flex items-center justify-between px-1.5 text-[10px] font-semibold uppercase tracking-[0.04em] text-[var(--color-text-tertiary)]">
            <span>当前研究问题的仿真</span>
            {loadingSimulations ? (
              <span className="material-symbols-outlined animate-spin text-[12px]" aria-hidden="true">progress_activity</span>
            ) : null}
          </div>
          {filteredSimulations.length > 0 ? (
            <div className="grid gap-1">
              {filteredSimulations.slice(0, 5).map((simulation) => (
                <button
                  key={simulation.id}
                  type="button"
                  data-testid="abm-reference-menu-item"
                  onClick={() => onSelectSimulation?.(simulation)}
                  className="flex min-w-0 items-center gap-2 rounded-lg px-2.5 py-2 text-left transition-colors hover:bg-[var(--color-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-brand)]/40"
                >
                  <span className="material-symbols-outlined shrink-0 text-[17px] text-[var(--color-brand)]" aria-hidden="true">
                    hub
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-[var(--color-text-primary)]">{simulation.name}</span>
                    <span className="block truncate font-[var(--font-mono)] text-[11px] text-[var(--color-text-tertiary)]">
                      {readModelId(simulation.config, simulation.name)} · v{readModelVersion(simulation)} · {simulation.id}
                    </span>
                  </span>
                  <span className="shrink-0 rounded-md border border-[var(--color-border)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--color-text-tertiary)]">
                    Simulation
                  </span>
                </button>
              ))}
            ...
            <div className="px-3 py-2 text-xs text-[var(--color-text-tertiary)]">
              {activeProjectId ? '当前研究问题下暂无匹配仿真。' : '采纳仿真方案后，可在这里引用。'}
            </div>
          )}
        </div>
      ) : null}
