...
export function StreamingIndicator() {
  ...
  return (
    <div className="mb-2 flex w-fit max-w-full flex-wrap items-center gap-2 rounded-full border border-[var(--color-border)]/40 bg-[var(--color-surface-container-low)] px-3 py-1">
      <ProcessStages chatState={chatState} streamingTokens={streamingTokens} statusVerb={statusVerb} />
...
function ProcessStages({
  ...
  const stage = chatState === 'tool_executing' || /run|tool|bash|search/i.test(statusVerb)
    ? 1
    : streamingTokens > 0
      ? 2
      : chatState === 'idle'
        ? 3
        : 0
  const labels = ['思考', '工具', '生成', '完成']
  return (
    <span className="flex shrink-0 items-center gap-1.5" aria-label={`当前阶段：${labels[stage]}`}>
          ...
          <span
            className={[
              ...
              index < stage
                ...
                : index === stage
                  ? 'animate-pulse-dot bg-[var(--color-brand)]'
                  : 'bg-[var(--color-border)]',
            ].join(' ')}
          />
          {index === stage ? <span className="sr-only">{label}</span> : null}
        </span>
      ))}
    </span>
  )
