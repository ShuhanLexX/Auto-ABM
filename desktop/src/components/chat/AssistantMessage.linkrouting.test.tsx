...
describe('AssistantMessage output-target cards', () => {
  it('hides final reply actions during autonomous ABM work until the session is idle', () => {
    const baseSession = useChatStore.getState().getSession('s1')
    useAbmStore.setState({ mode: 'autonomous' })
    useChatStore.setState({
      sessions: { s1: { ...baseSession, chatState: 'thinking' } },
    })

    render(<AssistantMessage sessionId="s1" content="阶段性说明" isStreaming={false} timestamp={Date.now()} />)

    expect(screen.queryByRole('button', { name: 'Copy reply' })).not.toBeInTheDocument()

    act(() => {
      useChatStore.setState({
        sessions: { s1: { ...baseSession, chatState: 'idle' } },
      })
    })
