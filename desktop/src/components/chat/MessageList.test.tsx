...
describe('MessageList nested tool calls', () => {
  ...
  it('rewinds and refills a completed user message from its edit action', async () => {
    vi.spyOn(sessionsApi, 'rewind').mockResolvedValueOnce({
      target: {
        targetUserMessageId: 'user-1',
        userMessageIndex: 0,
        userMessageCount: 1,
    ...
    const editActions = await screen.findAllByRole('button', { name: '撤回并编辑' })
    fireEvent.click(editActions[0]!)

    const dialog = await screen.findByRole('dialog', { name: '撤回并编辑这条消息？' })
    expect(within(dialog).getByText(/原消息会回到输入框/)).toBeTruthy()
    fireEvent.click(within(dialog).getByRole('button', { name: '撤回并编辑' }))

    await waitFor(() => {
      expect(sessionsApi.rewind).toHaveBeenCalledWith(ACTIVE_TAB, {
        targetUserMessageId: 'user-1',
        userMessageIndex: 0,
        expectedContent: '解释第一个仿真方案',
      })
    })
    expect(reloadHistory).toHaveBeenCalledWith(ACTIVE_TAB)
    expect(queueComposerPrefill).toHaveBeenCalledWith(ACTIVE_TAB, {
