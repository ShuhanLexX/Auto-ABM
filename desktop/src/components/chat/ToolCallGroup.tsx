...
const TOOL_VERBS: Record<string, (count: number, t: (key: TranslationKey, params?: Record<string, string | number>) => string, locale: string) => string> = {
  ...
  abm_propose_simulations: (n, _t, locale) => abmToolVerb(locale, 'Simulation drafts', '生成建模方案', n),
  abm_adopt_simulation: (n, _t, locale) => abmToolVerb(locale, 'Adopt simulation', '采纳仿真', n),
  abm_edit_model: (n, _t, locale) => abmToolVerb(locale, 'Edit model', '编辑模型', n),
  abm_run: (n, _t, locale) => abmToolVerb(locale, 'Run simulation', '运行仿真', n),
  abm_stop_run: (n, _t, locale) => abmToolVerb(locale, 'Stop run', '停止运行', n),
  abm_explain_interval: (n, _t, locale) => abmToolVerb(locale, 'Explain evidence', '解释证据', n),
  abm_configure_results: (n, _t, locale) => abmToolVerb(locale, 'Configure charts', '配置结果图', n),
  abm_configure_experiment_view: (n, _t, locale) => abmToolVerb(locale, 'Generate experiment UI', '生成实验 UI', n),
  abm_control_workbench: (n, _t, locale) => abmToolVerb(locale, 'Control workbench', '控制工作台', n),
  abm_inspect_simulation: (n, _t, locale) => abmToolVerb(locale, 'Review model', '审阅模型', n),
  abm_validate_simulation: (n, _t, locale) => abmToolVerb(locale, 'Validate model', '验证模型', n),
  abm_update_odd: (n, _t, locale) => abmToolVerb(locale, 'Update ODD', '更新 ODD', n),
  abm_attribute_interval: (n, _t, locale) => abmToolVerb(locale, 'Attribute interval', '机制归因', n),
  abm_counterfactual_run: (n, _t, locale) => abmToolVerb(locale, 'Counterfactual run', '反事实运行', n),
}

function abmToolVerb(locale: string, en: string, zh: string, count: number): string {
  return `${locale === 'zh' || locale === 'zh-TW' ? zh : en} (${count})`
}

function generateSummary(toolCalls: ToolCall[], t: (key: TranslationKey, params?: Record<string, string | number>) => string, locale: string): string {
  ...
  for (const [name, count] of counts) {
    ...
    parts.push(verbFn ? verbFn(count, t, locale) : `${name} (${count})`)
  }
...
function ToolCallGroupMulti({ toolCalls, resultMap, childToolCallsByParent, isStreaming }: Props) {
  ...
  const summary = generateSummary(toolCalls, t, locale)
