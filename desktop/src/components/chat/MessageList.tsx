...
import { ProposalBatch } from '../../abm/components/ProposalBatch'
import { ExplanationCard } from '../../abm/components/ExplanationCard'
import { AttributionCard } from '../../abm/components/AttributionCard'
import { CounterfactualCard } from '../../abm/components/CounterfactualCard'
...
function getMessageContentWeight(message: UIMessage): number {
  switch (message.type) {
    ...
    case 'abm_proposal_batch':
      return message.proposals.reduce(
        (total, proposal) =>
          total + proposal.mechanismSummary.length + proposal.expectedMacro.length + proposal.oddExcerpt.length,
        0,
      )
    case 'abm_explanation':
      return message.text.length + message.evidence.length * 8
    case 'abm_attribution':
      return (message.note?.length ?? 0) + message.contributions.length * 24
    case 'abm_counterfactual':
      return (message.note?.length ?? 0) + message.metrics.length * 24 + Object.keys(message.changed).length * 12
  }
}
...
function estimateMessageHeight(message: UIMessage): number {
  switch (message.type) {
    ...
    case 'abm_proposal_batch':
      return clampNumber(120 + message.proposals.length * 96, 140, 2600)
    case 'abm_explanation':
      return clampNumber(96 + Math.ceil(message.text.length / 90) * 20 + message.evidence.length * 26, 96, 2200)
    case 'abm_attribution':
      return clampNumber(150 + message.contributions.length * 56, 150, 1400)
    case 'abm_counterfactual':
      return clampNumber(200 + message.metrics.length * 24, 160, 1400)
    case 'error':
    case 'system':
      return 64
  }
...
function getMessageMetricSignature(message: UIMessage): string {
  switch (message.type) {
    ...
    case 'abm_proposal_batch':
      return `${message.type}:${message.proposals.length}:${message.proposals.map((proposal) => proposal.id).join(',')}`
    case 'abm_explanation':
      return `${message.type}:${message.text.length}:${message.evidence.length}:${message.speculative ? 1 : 0}`
    case 'abm_attribution':
      return `${message.type}:${message.runId}:${message.metric}:${message.contributions.length}:${message.supported ? 1 : 0}`
    case 'abm_counterfactual':
      return `${message.type}:${message.runId}:${message.status}:${message.metrics.length}`
  }
}
...
export function MessageList({ sessionId, compact = false }: MessageListProps = {}) {
  ...
  const [turnEditConfirmTargetId, setTurnEditConfirmTargetId] = useState<string | null>(null)
  ...
  const confirmEditTarget = useMemo(
    () => completedTurnTargets.find((target) => target.messageId === turnEditConfirmTargetId) ?? null,
    [completedTurnTargets, turnEditConfirmTargetId],
  )
  ...
  const handleConfirmEditTurn = useCallback(async () => {
    if (!resolvedSessionId || !confirmEditTarget || rewindingTurnId) return
    ...
    try {
      ...
      addToast({
        type: 'success',
        message: `已撤回 ${result.conversation.messagesRemoved} 条后续消息，原问题已放回输入框。`,
      })

      setTurnEditConfirmTargetId(null)
    } catch (error) {
      const detail = getApiErrorMessage(error)
      setTurnActionErrors((current) => ({
        ...current,
        [target.messageId]: detail,
      }))
      addToast({
        type: 'error',
        message: `撤回失败：${detail}`,
  ...
  const editActionByMessageId = useMemo(() => {
    if (isMemberSession || completedTurnTargets.length === 0) {
    ...
    for (const target of completedTurnTargets) {
      result.set(target.messageId, {
        label: '撤回并编辑',
        loading: rewindingTurnId === target.messageId,
        onEdit: () => setTurnEditConfirmTargetId(target.messageId),
      })
    }
    return result
  }, [completedTurnTargets, isMemberSession, rewindingTurnId])
    ...
    <div className="relative min-h-0 flex-1">
      ...
      >
        <div
          ...
          className={compact ? 'mx-auto w-full max-w-[760px]' : 'mx-auto max-w-[860px]'}
        >
      ...
      <ConfirmDialog
        ...
        onConfirm={handleConfirmEditTurn}
        title="撤回并编辑这条消息？"
        body="确认后，这条消息之后的回答、工具结果和仿真上下文都会被撤回，原消息会回到输入框供你修改后重新发送。"
        confirmLabel="撤回并编辑"
        cancelLabel={t('common.cancel')}
        confirmVariant="danger"
        loading={Boolean(rewindingTurnId)}
...
export const MessageBlock = memo(function MessageBlock({
  ...
  switch (message.type) {
    ...
    case 'abm_proposal_batch':
      return <ProposalBatch proposals={message.proposals} />
    case 'abm_explanation':
      return <ExplanationCard message={message} sessionId={sessionId} />
    case 'abm_attribution':
      return <AttributionCard message={message} />
    case 'abm_counterfactual':
      return <CounterfactualCard message={message} />
  }
})
