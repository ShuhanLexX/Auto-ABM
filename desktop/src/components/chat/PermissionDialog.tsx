...
import { ModelDiffPreview, parseModelDiff } from '../../abm/components/ModelDiffPreview'
...
const TOOL_META: Record<string, { icon: string; label: string; color: string }> = {
  ...
  abm_edit_model: { icon: 'account_tree', label: 'Edit Model', color: 'var(--color-brand)' },
}
...
function renderPermissionPreview(toolName: string, input: unknown) {
  ...
  if (toolName === 'abm_edit_model') {
    const diff = parseModelDiff(input)
    if (diff) return <ModelDiffPreview diff={diff} />
  }
