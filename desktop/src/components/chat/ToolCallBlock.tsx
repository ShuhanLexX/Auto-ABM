...
const TOOL_ICONS: Record<string, string> = {
  ...
  NotebookEdit: 'note',
  Skill: 'auto_awesome',
  abm_propose_simulations: 'schema',
  abm_adopt_simulation: 'science',
  abm_edit_model: 'account_tree',
  abm_run: 'play_circle',
  abm_stop_run: 'stop_circle',
  abm_explain_interval: 'auto_awesome',
  abm_configure_results: 'monitoring',
  abm_configure_experiment_view: 'tune',
  abm_control_workbench: 'dashboard_customize',
  abm_inspect_simulation: 'travel_explore',
  abm_validate_simulation: 'fact_check',
  abm_update_odd: 'article',
  abm_attribute_interval: 'balance',
  abm_counterfactual_run: 'fork_right',
}
...
function getToolDisplayName(toolName: string, locale: string): string {
  ...
  switch (toolName) {
    case 'abm_propose_simulations':
      return zh ? '建模方案 Skill' : 'Simulation Drafts Skill'
    case 'abm_adopt_simulation':
      return zh ? '采纳仿真 Skill' : 'Adopt Simulation Skill'
    case 'abm_edit_model':
      return zh ? '模型编辑 Skill' : 'Edit Model Skill'
    case 'abm_run':
      return zh ? '运行仿真 Skill' : 'Run Simulation Skill'
    case 'abm_stop_run':
      return zh ? '停止运行 Skill' : 'Stop Run Skill'
    case 'abm_explain_interval':
      return zh ? '证据解释 Skill' : 'Evidence Explanation Skill'
    case 'abm_configure_results':
      return zh ? '绘图 Skill' : 'Charting Skill'
    case 'abm_configure_experiment_view':
      return zh ? '实验 UI Skill' : 'Experiment UI Skill'
    case 'abm_control_workbench':
      return zh ? '工作台控制 Skill' : 'Workbench Control Skill'
    case 'abm_inspect_simulation':
      return zh ? '模型审阅 Agent' : 'Model Review Agent'
    case 'abm_validate_simulation':
      return zh ? '模型验证 Agent' : 'Model Validation Agent'
    case 'abm_update_odd':
      return zh ? 'ODD 写作 Skill' : 'ODD Writing Skill'
    case 'abm_attribute_interval':
      return zh ? '机制归因 Skill' : 'Mechanism Attribution Skill'
    case 'abm_counterfactual_run':
      return zh ? '反事实运行 Skill' : 'Counterfactual Run Skill'
    default:
      return toolName
  }
}
