...
describe('LocalSlashCommandPanel memory context', () => {
  ...
  it('shows a project name instead of the full working directory in status', async () => {
    const inspection = inspectionWithContext(baseContext)
    sessionsApiMock.getInspection.mockResolvedValue({
      ...inspection,
      status: {
        ...inspection.status,
        cwd: 'E:\\Users\\researcher\\AutoABM',
      },
    })
    ...
    expect(await screen.findByText('AutoABM')).toBeInTheDocument()
    expect(screen.queryByText('E:\\Users\\researcher\\AutoABM')).not.toBeInTheDocument()
  })
})
