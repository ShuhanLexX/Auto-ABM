...
function projectDisplayName(path?: string | null) {
  if (!path) return ''
...
function projectBadge(path?: string, t?: ReturnType<typeof useTranslation>) {
  ...
  const label = projectDisplayName(path)
      ...
      <section>
        ...
        <KeyValueRows
          rows={[
            ...
            [t('slash.inspector.status.workingDirectory'), <span className="font-mono text-[13px]">{projectDisplayName(data.status.cwd ?? data.status.workDir)}</span>],
...
function McpPanel({ cwd, onClose }: { cwd?: string; onClose: () => void }) {
  ...
  return (
    <PanelShell
      ...
      subtitle={cwd ? t('slash.mcp.subtitleWithProject', { path: projectDisplayName(cwd) }) : t('slash.mcp.subtitle')}
      onClose={onClose}
    >
      {error ? (
        <ErrorState message={error} />
...
function SkillsPanel({ cwd, onClose }: { cwd?: string; onClose: () => void }) {
  ...
  return (
    <PanelShell
      ...
      subtitle={cwd ? t('slash.skills.subtitleWithProject', { path: projectDisplayName(cwd) }) : t('slash.skills.subtitle')}
      onClose={onClose}
    >
      {error ? (
        <ErrorState message={error} />
