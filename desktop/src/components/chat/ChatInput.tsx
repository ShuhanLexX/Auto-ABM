...
import { MoonAttachButton, MoonComposerShell, MoonSendButton } from '../ui/moon-composer'
...
export function ChatInput({ variant = 'default', compact = false }: ChatInputProps) {
  ...
  useEffect(() => {
    ...
    let cursorPos = composerPrefill.text.length

    if (composerPrefill.mode === 'append') {
      const currentInput = inputRef.current
      const text = composerPrefill.text.trim()
      if (text) {
        const separator = currentInput.trim().length > 0 ? '\n\n' : ''
        const nextInput = `${currentInput}${separator}${text}`
  ...
  const selectAbmSimulationReference = useCallback((simulation: AbmSimulation) => {
    ...
    const next = insertComposerTokenAtRange(inputRef.current, atCursorPos, tokenEnd, replacement)
    setComposerInput(next.value)
    setFileSearchOpen(false)
    setAtFilter('')
    setAtCursorPos(-1)
  ...
  }, [atCursorPos, atFilter.length, setComposerInput])
  ...
  return (
    <div
      ...
      className={
        isHeroComposer
          ? `bg-transparent ${isMobileComposer ? 'px-4 pb-3' : 'px-8 pb-0'}`
          : compact
        ...
        >
          ...
          {isHeroComposer ? (
            <Textarea
              ...
              rows={2}
              className="min-h-[48px] border-none bg-transparent px-4 py-3 text-sm leading-relaxed shadow-none focus-visible:ring-0"
            ...
            <Textarea
              ...
              disabled={isWorkspaceMissing}
              rows={1}
              className={`border-none bg-transparent px-4 py-3 text-sm leading-relaxed shadow-none focus-visible:ring-0 ${
                useCompactControls ? 'min-h-[44px]' : 'min-h-[48px]'
              }`}
              style={{ fontFamily: 'var(--font-body)', overflow: 'hidden' }}
            />
          )}
          ...
          <div
            ...
            className={`moon-composer-footer flex items-center justify-between gap-3 p-3 ${
              useCompactControls ? 'px-2.5' : ''
            }`}
          >
                ...
                <>
                  ...
                  <div ref={plusMenuRef} className="relative">
                    <button
                      ...
                      className={`text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-hover)] ${isMobileComposer ? 'inline-flex h-11 w-11 items-center justify-center rounded-xl' : 'inline-flex h-9 w-9 items-center justify-center rounded-lg'}`}
                    >
                  ...
                  <AbmModeSelector compact={useCompactControls} />
                </>
              )}
            </div>
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <MoonSendButton
                active={!isMemberSession && isActive}
                disabled={!isMemberSession && isActive ? false : !canSubmit}
                onClick={!isMemberSession && isActive ? () => stopGeneration(activeTabId!) : handleSubmit}
                ariaLabel={!isMemberSession && isActive ? t('common.stop') : isMemberSession ? t('common.send') : t('common.run')}
                title={
