...
vi.mock('../../abm/components/AbmModeSelector', () => ({
  AbmModeSelector: () => <button type="button">研究</button>,
}))
...
describe('ChatInput file mentions', () => {
  ...
  it('shows branch and worktree launch controls for an empty active Git session', async () => {
    ...
    expect(await screen.findByRole('button', { name: /Select branch: main/, hidden: true })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Select worktree mode: Current worktree/, hidden: true })).toBeInTheDocument()
    const branchButton = screen.getByRole('button', { name: /Select branch: main/, hidden: true })
    expect(branchButton.closest('[aria-hidden="true"]')).toBeTruthy()
  })
  ...
  it('starts an empty active session on the selected branch without an isolated worktree', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /Select branch: main/, hidden: true }))
  ...
  it('starts an empty active session on the selected branch inside an isolated worktree', async () => {
    ...
    expect(screen.getByRole('button', { name: /Select worktree mode: Isolated worktree/, hidden: true })).toBeInTheDocument()
    const input = screen.getByRole('textbox') as HTMLTextAreaElement
  ...
  it('inserts current research simulations from the @ menu', async () => {
    ...
    fireEvent.change(input, {
      target: {
        value: '@rumor 请解释机制。',
        selectionStart: '@rumor'.length,
      },
    })

    fireEvent.click(await screen.findByText('谣言隔离模型'))

    await waitFor(() => {
      expect(input.value).toBe('@Simulation 谣言隔离模型 (id=sim-rumor) 请解释机制。')
    })
  ...
  it('uses larger icon-only mobile action buttons for browser H5 access', async () => {
    ...
    expect(screen.getByTestId('chat-input-panel')).toHaveClass('moon-composer')
  ...
  it('keeps the active-session toolbar in flow so multiline caret cannot render behind controls', async () => {
    ...
    expect(toolbar).toHaveClass('moon-composer-footer')
