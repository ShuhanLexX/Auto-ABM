...
function buildBreadcrumbs(filePath: string): Array<{ label: string; path: string }> {
  if (!filePath) return []

  if (isWindowsAbsolutePath(filePath)) {
    const drive = filePath.slice(0, 2)
    const tail = filePath.slice(2).replace(/^[\\/]+/, '')
    const parts = tail.split(/[\\/]+/).filter(Boolean)
    const crumbs: Array<{ label: string; path: string }> = [{ label: drive, path: `${drive}\\` }]
    let current = `${drive}\\`
...
export function DirectoryPicker({ value, onChange, variant = 'chip', isGitProject = false }: Props) {
  ...
  const loadBrowseDir = async (path?: string) => {
    ...
    try {
      const result = await filesystemApi.browse(path, { directoryPicker: true })
  ...
  const dropdownContent = mode === 'recent' ? (
    <>
      ...
      <div className="border-t border-[var(--color-border)]">
        ...
        >
          <span className={`material-symbols-outlined text-[20px] text-[var(--color-text-tertiary)] ${folderDialogLoading ? 'animate-spin' : ''}`}>
            {folderDialogLoading ? 'progress_activity' : 'create_new_folder'}
          </span>
          <span className="text-sm text-[var(--color-text-secondary)]">
            {folderDialogLoading ? t('common.loading') : t('dirPicker.chooseFolder')}
          </span>
        </button>
      </div>
    </>
    ...
    <>
      <div className="flex flex-wrap items-center gap-1 border-b border-[var(--color-border)] px-3 py-2">
        ...
        <button onClick={() => loadBrowseDir(undefined)} className="text-[10px] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)]">
          {t('dirPicker.computer')}
        </button>
      ...
      <div className={`${isMobileBrowser ? '' : 'max-h-[240px]'} overflow-y-auto`}>
        {browseLoading ? (
          ...
          <>
            {browsePath && browseParent !== browsePath && (
              <button onClick={() => loadBrowseDir(browseParent)} className="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-[var(--color-surface-hover)]">
                ...
                <span className="text-xs text-[var(--color-text-secondary)]">{browseParent ? '..' : t('dirPicker.computer')}</span>
              </button>
            )}
      ...
      <div className="flex items-center justify-between border-t border-[var(--color-border)] px-3 py-2">
        <span className="truncate font-[var(--font-mono)] text-[10px] text-[var(--color-text-tertiary)]">{browsePath || t('dirPicker.computer')}</span>
        <button
          onClick={() => browsePath && handleSelect(browsePath)}
          disabled={!browsePath || browseLoading}
          className="rounded-lg bg-[var(--color-brand)] px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-45"
        >
