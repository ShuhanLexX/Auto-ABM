...
// ─── ABM card payloads (P2) ───────────────────────────────────────
// Mirror of the tool↔desktop contract produced by src/tools/abm/* agent tools.

export type AbmProposal = {
  id: string
  mechanismSummary: string
  keyParams: Record<string, unknown>
  expectedMacro: string
  oddExcerpt: string
  // Only present when a real low-step trial run was performed (real run_id).
  trial?: { runId: string; sparkline: number[] }
}

// Mirror of src/server/abm/explainService.ts::Evidence.
export type AbmEvidence = {
  tick: number
  metric?: string
  value?: number
  event?: string
...
// Mirror of src/tools/abm/abmCardEnvelope.ts (attribution card).
export type AbmAttributionContribution = {
  mechanism_id: string
  gains: number
  losses: number
  net: number
...
// Mirror of src/tools/abm/abmCardEnvelope.ts (counterfactual card).
export type AbmComparisonMetric = {
  metric: string
...
export type UIMessage =
  ...
  | { id: string; type: 'abm_proposal_batch'; proposals: AbmProposal[]; timestamp: number }
  | {
      id: string
