...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/autoabm-data')
    ...
    useSettingsStore.setState({
      ...
      traceCapture: { enabled: true, storageDir: '/Users/test/.claude/autoabm/traces' },
      ...
      appMode: {
        mode: 'default',
        portableDir: null,
        defaultPortableDir: '/Applications/AutoABM/AUTOABM_DATA',
      ...
      setAppMode: vi.fn().mockImplementation(async (mode: AppMode, portableDir?: string | null) => {
        useSettingsStore.setState({
          appMode: {
            mode,
            portableDir: mode === 'portable' ? portableDir ?? '/Applications/AutoABM/AUTOABM_DATA' : null,
            defaultPortableDir: '/Applications/AutoABM/AUTOABM_DATA',
            activeConfigDir: mode === 'portable' ? portableDir ?? '/Applications/AutoABM/AUTOABM_DATA' : null,
  ...
  it('lets desktop users choose a portable data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Portable data directory')).toHaveValue('/Users/test/autoabm-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/autoabm-data')
  ...
  it('switches back to the system directory without deleting portable data', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/autoabm-data',
        defaultPortableDir: '/Applications/AutoABM/AUTOABM_DATA',
        activeConfigDir: '/Users/test/autoabm-data',
  ...
  it('validates portable directory input and lets users reset to the app-side folder', async () => {
    ...
    expect(input).toHaveValue('/Applications/AutoABM/AUTOABM_DATA')
  ...
  it('treats external AUTOABM_CONFIG_DIR as the controlling data source', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
    ...
    fireEvent.click(screen.getByText('General'))
    expect(screen.getByText(/The current directory is controlled by the AUTOABM_CONFIG_DIR environment variable/)).toBeInTheDocument()
  ...
  it('requests native notification permission when desktop notifications are enabled', async () => {
    ...
    expect(desktopNotificationsMock.notifyDesktop).toHaveBeenCalledWith({
      title: 'AutoABM notifications are enabled',
...
describe('Settings > About tab', () => {
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'AutoABM v0.1.5' })).toBeInTheDocument()
