...
describe('Content-only pages render without errors', () => {
  ...
  it('EmptySession renders mascot and composer', async () => {
    ...
    expect(container.innerHTML).toContain('AutoABM · AI for Social Science')
    expect(container.innerHTML).toContain('Automate agent-based model design, experiments, and discovery')
  })
  ...
  it('EmptySession plus menu exposes uploads and slash commands before chat starts', async () => {
    ...
    expect(screen.getByText('Add data or papers')).toBeInTheDocument()
  ...
  it('ActiveSession renders with chat components', () => {
    ...
    expect(textarea).toHaveAttribute('placeholder', 'Describe the phenomenon or system you want to study...')
  ...
  it('ActiveSession keeps the compact composer once messages exist', () => {
    ...
    const textarea = screen.getByPlaceholderText('Ask AutoABM to model, run, compare or explain...')
...
describe('AppShell layout renders chrome', () => {
  it('AppShell renders sidebar and session shell', () => {
    ...
    expect(container.innerHTML).toContain('New research session')
