...
export const ABM_KERNEL_ROOT_ENV = 'ABM_KERNEL_ROOT'
...
export function resolveBundledAbmKernelRoot(desktopRoot: string): string {
  return path.join(desktopRoot, 'src-tauri', 'resources', 'abm-kernel')
}
...
export function claudeConfigDir(env: NodeJS.ProcessEnv = process.env): string {
  return getAppConfigHomeDir(env)
}

/** Parse h5Access.fixedPort out of autoabm/settings.json contents. */
...
export function readH5FixedPort(env: NodeJS.ProcessEnv = process.env): number | null {
  try {
    const settingsPath = path.join(appStateDir(claudeConfigDir(env)), 'settings.json')
...
export function buildSidecarEnv(
  ...
  if (abmKernelRoot) {
    env[ABM_KERNEL_ROOT_ENV] = baseEnv[ABM_KERNEL_ROOT_ENV] || abmKernelRoot
  }
  const configDir = resolveConfigDirFromEnv(baseEnv)
  if (configDir) {
    const cacheDir = path.join(configDir, 'Cache')
    mkdirSync(cacheDir, { recursive: true })
    env.AUTOABM_CONFIG_DIR = configDir
...
export function createServerPlan({
  ...
  return {
    ...
    env: buildSidecarEnv(env, h5DistDir, resolveBundledAbmKernelRoot(desktopRoot)),
  }
}
