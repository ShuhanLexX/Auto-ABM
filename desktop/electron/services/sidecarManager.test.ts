...
describe('Electron sidecar manager', () => {
  ...
  it('builds server sidecar args without changing the REST/WebSocket boundary', () => {
    ...
    expect(plan.env.ABM_KERNEL_ROOT).toBe(path.join('/app/desktop', 'src-tauri', 'resources', 'abm-kernel'))
  })
  ...
  it('can keep sidecar binaries and H5 assets unpacked while pointing app-root at app.asar', () => {
    ...
    expect(normalizePathSeparators(plan.command)).toContain('/Applications/App.app/Contents/Resources/app.asar.unpacked/src-tauri/binaries/claude-sidecar-')
    expect(plan.args).toContain('/Applications/App.app/Contents/Resources/app.asar')
    expect(normalizePathSeparators(plan.env.CLAUDE_H5_DIST_DIR ?? '')).toBe('/Applications/App.app/Contents/Resources/app.asar.unpacked/dist')
    expect(normalizePathSeparators(plan.env.ABM_KERNEL_ROOT ?? '')).toBe('/Applications/App.app/Contents/Resources/app.asar.unpacked/src-tauri/resources/abm-kernel')
  })

  it('passes portable config and adapter server URL through the sidecar env', () => {
    const configDir = mkdtempSync(path.join(tmpdir(), 'autoabm-config-'))
    try {
      ...
      const server = createServerPlan({
        desktopRoot: '/app/desktop',
        appRoot: '/app',
        port: 4567,
        env: { CLAUDE_CONFIG_DIR: configDir, ABM_KERNEL_ROOT: '/custom/abm-kernel' },
      })
      expect(server.env.ABM_KERNEL_ROOT).toBe('/custom/abm-kernel')
  ...
  it('persists and prioritizes preferred server ports from the config dir', () => {
    ...
    try {
      ...
      mkdirSync(path.join(configDir, 'autoabm'), { recursive: true })
      writeFileSync(
        path.join(configDir, 'autoabm', 'settings.json'),
