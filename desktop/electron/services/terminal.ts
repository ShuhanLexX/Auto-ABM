...
import { getAppConfigHomeDir, resolveConfigDirFromEnv } from '../../../src/constants/appPaths.js'
import { ELECTRON_EVENT_CHANNELS } from '../ipc/channels'

const TERMINAL_CONFIG_FILE = 'terminal-config.json'
const MIN_TERMINAL_COLS = 20
const MIN_TERMINAL_ROWS = 8
const NODE_PTY_MANIFEST_FILE = '.autoabm-node-pty-manifest.json'
...
export function terminalConfigPath(app: TerminalAppLike | undefined, env: NodeJS.ProcessEnv = process.env): string | null {
  const configDir = resolveConfigDirFromEnv(env)
  if (configDir) {
    return path.join(configDir, TERMINAL_CONFIG_FILE)
  }
  if (!app) return null
