...
describe('release desktop workflow', () => {
  ...
  test('release workflow installs uv before building ABM kernel resources', () => {
    const workflow = readReleaseWorkflow()
    const setupUvStep = extractStep(workflow, 'Setup uv')

    expect(setupUvStep).toContain('astral-sh/setup-uv@v8')
    expect(setupUvStep).toContain("version: '0.11.24'")
    expect(setupUvStep).toContain('enable-cache: true')
    expect(workflow.indexOf('Setup uv')).toBeLessThan(workflow.indexOf('Build sidecars'))
  })
  ...
  test('desktop package includes Linux deb metadata required by electron-builder', () => {
    ...
    expect(desktopPackage.homepage).toBe('https://github.com/SocialAI-X/Auto-ABM')
    expect(desktopPackage.author?.name).toBe('AutoABM')
    expect(desktopPackage.author?.email).toBe('relakkes@gmail.com')
    expect(desktopPackage.build?.linux?.maintainer).toBe('AutoABM <relakkes@gmail.com>')
  })
  ...
  test('release workflow signs and notarizes macOS builds only when signing preflight succeeds', () => {
    ...
    expect(signedBuildStep).toContain('app_path="build-artifacts/electron/${{ matrix.app_bundle_dir }}/AutoABM.app"')
  ...
  test('release matrix asset basenames remain unique when final artifacts are flattened', () => {
    ...
    expect(desktopPackage.build.artifactName).toBe('AutoABM-${version}-${os}-${arch}.${ext}')

    const expectedReleaseAssets = [
      `AutoABM-${version}-mac-arm64.dmg`,
      `AutoABM-${version}-mac-arm64.dmg.blockmap`,
      `AutoABM-${version}-mac-arm64.zip`,
      `AutoABM-${version}-mac-arm64.zip.blockmap`,
      `AutoABM-${version}-mac-x64.dmg`,
      `AutoABM-${version}-mac-x64.dmg.blockmap`,
      `AutoABM-${version}-mac-x64.zip`,
      `AutoABM-${version}-mac-x64.zip.blockmap`,
      `AutoABM-${version}-linux-x86_64.AppImage`,
      `AutoABM-${version}-linux-amd64.deb`,
      `AutoABM-${version}-linux-arm64.AppImage`,
      `AutoABM-${version}-linux-arm64.deb`,
      `AutoABM-${version}-win-x64.exe`,
      `AutoABM-${version}-win-x64.exe.blockmap`,
    ]
  ...
  test('release workflow validates exact expected release assets and update metadata before publishing', () => {
    ...
    const publishJob = extractJob(workflow, 'publish-release')
    const expectedFiles = [
      'AutoABM-${APP_VERSION}-mac-arm64.dmg',
      'AutoABM-${APP_VERSION}-mac-arm64.zip',
      'AutoABM-${APP_VERSION}-mac-x64.dmg',
      'AutoABM-${APP_VERSION}-mac-x64.zip',
      'AutoABM-${APP_VERSION}-linux-x86_64.AppImage',
      'AutoABM-${APP_VERSION}-linux-amd64.deb',
      'AutoABM-${APP_VERSION}-linux-arm64.AppImage',
      'AutoABM-${APP_VERSION}-linux-arm64.deb',
      'AutoABM-${APP_VERSION}-win-x64.exe',
    ]
  ...
  test('Electron package keeps ABM kernel resources unpacked for Python', () => {
    const desktopPackage = JSON.parse(readFileSync('desktop/package.json', 'utf8')) as {
      build: {
        asarUnpack?: string[]
        files?: string[]
    ...
    expect(desktopPackage.build.files).toContain('src-tauri/resources/abm-kernel/**')
    expect(desktopPackage.build.asarUnpack).toContain('src-tauri/resources/abm-kernel/**')
  })
