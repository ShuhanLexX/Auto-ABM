...
function addAbmKernelChecks(
  report: PackageSmokeReport,
  rootDir: string,
  labelPrefix: string,
  unpackedResourcesDir: string,
) {
  const kernelDir = join(unpackedResourcesDir, 'src-tauri', 'resources', 'abm-kernel')
  addPresenceCheck(report, rootDir, `${labelPrefix} ABM kernel pyproject`, join(kernelDir, 'pyproject.toml'))
  addPresenceCheck(report, rootDir, `${labelPrefix} ABM kernel worker`, join(kernelDir, 'src', 'abm_kernel', 'worker.py'))
  addMatchCheck(
    report,
    rootDir,
    `${labelPrefix} ABM kernel Python runtime`,
    findMatches(join(kernelDir, '.python'), (candidate) => {
      const normalized = normalizePath(candidate)
      return normalized.endsWith('/python.exe') || normalized.endsWith('/bin/python3') || normalized.endsWith('/bin/python')
    }),
    join(kernelDir, '.python'),
  )
  addMatchCheck(
    report,
    rootDir,
    `${labelPrefix} ABM kernel Mesa dependency`,
    findMatches(join(kernelDir, '.venv'), (candidate) => normalizePath(candidate).endsWith('/site-packages/mesa/__init__.py')),
    join(kernelDir, '.venv'),
  )
}
...
function inspectMacosArtifacts(rootDir: string, report: PackageSmokeReport, options: InspectOptions) {
  ...
  addAbmKernelChecks(report, rootDir, 'macOS unpacked', unpackedDir)
...
function inspectWindowsArtifacts(rootDir: string, report: PackageSmokeReport) {
  ...
  if (unpackedDir) {
    ...
    addAbmKernelChecks(report, rootDir, 'Windows unpacked', unpackedResourcesDir)
    addInstalledUpdateMetadataCheck(
      report,
      rootDir,
...
function inspectLinuxArtifacts(rootDir: string, report: PackageSmokeReport) {
  ...
  if (unpackedDir) {
    ...
    addAbmKernelChecks(report, rootDir, 'Linux unpacked', unpackedResourcesDir)
    addInstalledUpdateMetadataCheck(
      report,
      rootDir,
