...
export async function executeDesktopSmoke(
  ...
  try {
    ...
    const browserSetup = [
      `localStorage.setItem('autoabm-open-tabs', ${JSON.stringify(JSON.stringify({
        openTabs: [{ sessionId: session.sessionId, title: 'Desktop Smoke', type: 'session' }],
        activeTabId: session.sessionId,
      }))})`,
      runtimeSelection
        ? `localStorage.setItem('autoabm-session-runtime', ${JSON.stringify(JSON.stringify({
          [session.sessionId]: runtimeSelection,
        }))})`
        : `localStorage.removeItem('autoabm-session-runtime')`,
    ]
